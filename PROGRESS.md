...
\textbf{Translation state (post-Cowork-session-53):} \textbf{925 /
958 works drafted (\~96.6\%)} --- session 53 continued \textbf{Phase 3
(the major-works sweep)} by translating the single chronologically-next
fetchable major work: \textit{Brutus} (46 BC), Cicero's history of Roman
oratory (311 sections present in Perseus's numbering 1--333,
$\sim$38{,}000 English words). With \textit{Pro Milone} done last session
and the two manual-source snags (\textit{de-legibus},
\textit{ad-familiares-05-10}) deliberately skipped, this was the next
non-deferred fetchable work; the chronology pointer now advances into the
46--44 BC rhetorica/philosophica run.

\textbf{Method (parallel, by section-range):} the PM pre-fetched
\textit{Brutus} via \texttt{scripts/fetch\_latin.py} (Perseus TEI
\texttt{phi0474.phi039}, 175{,}585 chars, 311 sections, clean 1--333 run
with the standard scholarly numbering gaps), sliced the Latin into twelve
contiguous range files in the Cowork \emph{outputs} scratch dir, and
dispatched twelve worker sub-agents in \textbf{three parallel batches of
four} (\textsection\textsection 1--26, 27--52, 53--81, 82--107 /
108--135, 136--163, 164--190, 191--219 / 220--248, 249--278, 279--305,
306--333), all under \texttt{055bc-de-oratore} as the register/voice
template. Each worker emitted only its English part file to scratch; the
PM assembled the twelve parts into
\texttt{english/rhetoric/046bc-brutus.tex} (header added on assembly),
\textbf{verified the assembled section sequence is identical to the
Latin's} (311 markers, exact 1--333 run with all gaps preserved --- one
spurious inline \textsection 282 marker introduced by the
\textsection\textsection 279--305 worker was caught in the marker-diff
check and removed), wrote the whole-work headnote itself (whole-work
understanding needed), and flipped the status. Per-work parallel sidecar
generated by \texttt{backfill\_structural.py} (311 segments, locs
1--333, zero empty segs).

\textbf{Date / Greek / register checks:} \textbf{No date correction} ---
\textit{Brutus} carries no Perseus dateline; the conventional early-46 BC
dating is retained (\texttt{date: -0046-04-01}, precision \texttt{month})
and the file is correctly named \texttt{046bc-}. \textbf{Eight Greek
phrases} occur and are rendered meaning-first with \textit{[Greek:
\ldots]} (\textit{Peith\=o} \textsection 59; \textit{tropoi} /
\textit{sch\=emata} \textsection 69; \textit{periodon} / \textit{k\=ola}
\textsection 162; \textit{politikon} \textsection 265; \textit{sch\=emata}
\textsection 275). Speaker turns (Cicero / Brutus / Atticus) are rendered
inline as narrative dialogue per the \texttt{de-oratore} convention, not
as \texttt{\textbackslash textsc\{\}} catechism labels.

\textbf{Translator-notes worth logging in
\texttt{data/translator-notes.jsonl}} (deferred to a future enrichment
pass to avoid concurrent-write contention):
\begin{itemize}
\item \textbf{\textsection\textsection 16--19} --- the sustained
debt/dunning conceit (\textit{flagitator}) for the ``gift'' of writing
Cicero owes Brutus; the financial metaphor kept running across the
sequence rather than flattened.
\item \textbf{\textsection\textsection 58--59} --- the Ennius
\textit{Annales} fragment on Cethegus (\textit{Suadae medulla}, ``the
marrow of Persuasion''), set as verse; \textit{Suada} kept as a proper
name beside the Greek gloss \textit{Peith\=o}.
\item \textbf{\textsection 160} --- \textit{Liciniam virginem} rendered
``the Vestal Licinia'' (interpretive expansion; the Latin says only ``the
maiden Licinia''). \textbf{Flag for editorial review} --- the volume may
prefer the literal.
\item \textbf{\textsection 192} --- \textit{scalmus} ``thole-pin,''
Crassus's witticism (a whole ship raised from one peg, mirroring Scaevola
building a whole suit from one quibble); literal nautical image kept.
\item \textbf{\textsection 260} --- \textit{sputatilica}, an
untranslatable coinage/pun, rendered ``spittable'' with the joke split
(``spit I know; -table I do not'') to keep Rusius's mockery legible.
\item \textbf{\textsection 283} --- \textit{verum sanguinem
deperdebat}, of Calvus's over-thinned style, ``kept losing even his true
blood''; the literal blood-image kept.
\item \textbf{\textsection\textsection 330--333} --- the closing
apostrophe to Brutus: the ``guardians of an orphaned eloquence'' allegory
kept its full image, and the text deliberately ends on the genuine broken
final sentence (\textit{si mihi accidisset\ldots}) rather than smoothed
into closure --- the work falls silent in the act of recording its own
history.
\item \textbf{Daggered textual cruxes} in Perseus (e.g.\ \textsection
46, \textsection 175, \textsection 206 ``Q. Metello \dag F.'',
\textsection 207 \dag Cotta\dag, \textsection 233 \dag admirando\dag,
\textsection 315) rendered through to the standard sense; obeli preserved
in the English where a name or word is genuinely lost.
\end{itemize}

\textbf{Validation:} \texttt{validate.py} clean --- exit 0, 8 warnings =
the pre-existing orphan-Latin (\texttt{054bc-ad-familiares-13-49}) plus
the chronological-gap warning, now at \textbf{27} pending earlier-dated
works (down from 28 post-session-52, the expected $-1$ decrement since
\textit{Brutus} at 46 BC fell inside the chronological-gap zone).
\textbf{Latest drafted by deep chronology is unchanged}: \textit{Ad
...
\textbf{Suggested next translation batch} (when session 53 is landed via
\texttt{bash scripts/cowork\_handoff.sh "session 53: \ldots"} and session
54 opens): continue the \textbf{46--44 BC rhetorica/philosophica run}.
\textbf{The two manual-source snags remain at the front of the chronology
and are NOT fetchable} --- \textit{de-legibus} (52 BC, complete but
absent from Perseus's TEI; the old \texttt{thelatinlibrary.com/cicero/leg.shtml}
source 403s) and \textit{ad-familiares-05-10} (51 BC, absent from Perseus
\texttt{phi056}). Do NOT run \texttt{fetch\_latin.py} on these; flag for
Alexander to supply Latin manually at a convenient handoff. \textbf{Next
fetchable works in order}: \textit{ad-familiares-09-25} (46 BC, a short
letter to Papirius Paetus --- quick), then
\textit{de-optimo-genere-oratorum} (46), \textit{paradoxa-stoicorum}
(46), \textit{orator} (46), \textit{academica} (45), \textit{de-finibus}
(45), \textit{tusculanae-disputationes} (45), \textit{de-natura-deorum}
(45), \textit{de-officiis} (44). The long treatises in that run
(\textit{Orator}, \textit{De Finibus}, \textit{Tusculans}, \textit{De
Natura Deorum}, \textit{De Officiis}) suit the parallel-worker-by-section-range
pattern used here for \textit{Brutus}.

\textbf{No-Perseus / deferred works} remaining (do NOT
\texttt{fetch\_latin.py}): \textit{de-legibus} (52, complete, manual),
\textit{ad-familiares-05-10} (51, Perseus-absent),
\textit{de-consulatu-suo} (60), \textit{de-temporibus-suis} (54),
\textit{hortensius} (45), \textit{consolatio} (45) --- fragmentary; plus
the deliberately-skipped pre-\textit{Pro Quinctio} works \textit{aratea}
(86) and \textit{de-inventione} (85). \textbf{Slice M} (entity-stub
enrichment, 429+ stubs) remains the parallel-friendly fallback when no
fetchable substantial work is ready.

\textbf{File carry-overs:} the \textit{Brutus} Latin slices and English
part files (\texttt{brutus-latin-p01..p12},
\texttt{046bc-brutus-p01..p12-en}) were written to the Cowork
\emph{outputs} scratch directory (outside the repo), so they do NOT
pollute the working tree --- no new in-repo carry-overs this session.
Earlier carry-overs persist (sandbox cannot delete; Alexander handles at
handoff): \textit{Att.} 5.1, 5.7 (\texttt{049bc-} $\rightarrow$
\texttt{051bc-}), \textit{Fam.} 13.09 (\texttt{005bc-} $\rightarrow$
\texttt{050bc-}), and the orphan \texttt{054bc-ad-familiares-13-49} Latin.

\textbf{Launch-prompt advisory:} the scheduled 2-hour Cowork prompt
(``dispatch 3--4 parallel batches \ldots 3--5 short letters'') remains
stale and continues to auto-pivot. The project is in \textbf{Phase 3
(major-works sweep)}: one (or a few contiguous) substantial work(s) per
session, parallelized by section-range. The PM in each scheduled session
reads ``Where to resume now'' first and follows what it says, so this is
not blocking --- but updating the scheduled-task prompt to Phase 3 wording
is still recommended.

---

## Where to resume now (session 52 legacy snapshot below)

\textbf{Translation state (post-Cowork-session-52):} \textbf{924 /
958 works drafted (\~96.4\%)} --- session 52 continued \textbf{Phase 3
(the major-works sweep)} by clearing the single chronologically-next
fetchable major speech: \textit{Pro Milone} (105 sections,
$\sim$15{,}560 English words). With \textit{Pro Plancio} done last
session, this was the next non-deferred pending work; the 52 BC
\textit{vis}-trial speech is now drafted and the chronology pointer
advances past it into a run of manual-source snags (see ``next batch''
below).

\textbf{Method (parallel, by section-range):} the PM pre-fetched
\textit{Pro Milone} via \texttt{scripts/fetch\_latin.py} (Perseus TEI
\texttt{phi0474.phi031}, 73{,}006 chars, 105 sections, clean 1..105
run), sliced the Latin into four contiguous range files in the Cowork
\emph{outputs} scratch dir, and dispatched four worker sub-agents in
one batch (\textsection\textsection 1--26, 27--53, 54--79, 80--105),
all under \texttt{054bc-pro-plancio} as the register/voice template.
Each worker emitted only its English part file to scratch; the PM
assembled the four parts into
\texttt{english/speeches/052bc-pro-milone.tex} (header added on
assembly, exact 1..105 sequence verified --- no gaps/dupes), wrote the
whole-speech headnote itself (whole-speech understanding needed), and
flipped the status. Per-work parallel sidecar was generated by
\texttt{backfill\_structural.py} (105 segments, locs 1--105, zero empty
English segs) rather than emitted by workers, to remove a
JSON-malformation risk; backfill aligns mechanically on
\texttt{\textbackslash ciceroSection} labels.

\textbf{Date / Greek / register checks:} \textbf{No date correction} ---
speeches carry no Perseus dateline; the conventional April 52 BC dating
is retained (\texttt{date: -0052-04-01}, precision \texttt{month}) and
the file is correctly named \texttt{052bc-}. \textbf{No Greek phrases}
occur anywhere in the speech. Oath/religion register checked against the
template: \textit{me hercule} $\rightarrow$ ``by Hercules,''
\textit{me dius fidius} $\rightarrow$ ``so help me god'' (lowercase,
matching \texttt{pro-plancio}), and the \textsection 85 apostrophe to
the Alban hills / Jupiter of Latium kept fully Roman (not
Christianized).
...
pass to avoid concurrent-write contention):
\begin{itemize}
\item \textbf{\textsection 11} --- the famous \textit{silent enim leges
inter arma} rendered freshly as ``amid arms the laws fall silent''
(deliberately not echoing the canonical ``in time of war the laws fall
silent''); Cicero's word order kept so the antithesis lands where his
does.
\item \textbf{\textsection\textsection 10--11} --- \textit{non scripta
sed nata lex}, the unwritten/inborn natural-law passage; the asyndetic
tricola (\textit{adripuimus, hausimus, expressimus}; \textit{docti/facti,
instituti/imbuti}) preserved as English doublets to carry the build.
\item \textbf{\textsection 15} --- the ballot tablets A (\textit{absolvo})
/ C (\textit{condemno}) rendered ``letter of acquittal'' / ``that grim
one of condemnation''; literal ``letter'' would baffle a modern reader.
\textbf{Glossary candidate}: the Roman voting letters.
\item \textbf{\textsection 30} --- \textit{vi victa vis vel potius
oppressa virtute audacia est}, dense alliterative chiasmus, rendered
``force was conquered by force, or rather audacity was crushed by
valour'' (force/valour, force/audacity antithesis kept without forcing
English alliteration).
\item \textbf{\textsection 32} --- the Cassian \textit{cui bono} maxim
rendered as the recognizable principle ``to whose benefit,'' not left as
a bare Latin tag.
\item \textbf{\textsection 57} --- \textit{facti...quaestio est, iuris
in iudicio}: ``fact on the rack, law in court,'' \textit{eculeo}
(``little horse'' rack) kept as plain ``rack.''
\item \textbf{\textsection 105} --- \textit{O terram illam beatam}: the
asyndetic tricolon of conditional exclamations (\textit{exceperit /
eiecerit / amiserit}) preserved as a broken, suspended cadence rather
than smoothed into one sentence.
\end{itemize}

\textbf{Validation:} \texttt{validate.py} clean --- 8 warnings = the
pre-existing orphan-Latin (\texttt{054bc-ad-familiares-13-49}) plus the
chronological-gap warning, now at \textbf{28} pending earlier-dated
works (down from 29 post-session-51, the expected $-1$ decrement since
\textit{Pro Milone} at 52 BC fell inside the chronological-gap zone).
\textbf{Latest drafted by deep chronology is unchanged}: \textit{Ad
Familiares} 10.24 (28 July 43 BC) remains the marker.
\texttt{build\_manifest.py} regenerated \texttt{MANIFEST.md} (958
entries) and \texttt{build/chronology.tex}.

\textbf{Suggested next translation batch} (when session 52 is landed
via \texttt{bash scripts/cowork\_handoff.sh "session 52: \ldots"} and
session 53 opens):

\textbf{Heads-up --- two manual-source snags now sit at the front of
the chronology and are NOT fetchable.} The two chronologically-next
non-deferred pending works are \textit{de-legibus} (52 BC, complete but
absent from Perseus's TEI corpus --- needs manual Latin; the old
\texttt{thelatinlibrary.com/cicero/leg.shtml} source currently 403s, so
another classical-Latin repository must supply it) and
\textit{ad-familiares-05-10} (51 BC, absent from Perseus
\texttt{phi056} --- Latin source TBD; skip or source manually). Do NOT
run \texttt{fetch\_latin.py} on these; it will fail or dump wrong
content. \textbf{Recommended for an autonomous session}: skip both
manual snags and resume the great \textbf{46--44 BC
rhetorica/philosophica run}, whose first fetchable work is
\textit{brutus} (46 BC), followed by \textit{ad-familiares-09-25},
\textit{de-optimo-genere-oratorum}, \textit{paradoxa-stoicorum},
\textit{orator}, \textit{academica}, \textit{de-finibus},
\textit{tusculanae-disputationes}, \textit{de-natura-deorum},
\textit{de-officiis}. The long treatises in that run suit the
parallel-worker-by-section-range pattern used here. Flag
\textit{de-legibus} + \textit{ad-familiares-05-10} for Alexander to
supply Latin manually at a convenient handoff.

\textbf{No-Perseus / deferred works} remaining (do NOT
\texttt{fetch\_latin.py}): \textit{de-legibus} (52, complete, manual),
\textit{ad-familiares-05-10} (51, Perseus-absent),
\textit{de-consulatu-suo} (60), \textit{de-temporibus-suis} (54),
\textit{hortensius} (45), \textit{consolatio} (45) --- fragmentary;
plus the deliberately-skipped pre-\textit{Pro Quinctio} works
\textit{aratea} (86) and \textit{de-inventione} (85). \textbf{Slice M}
(entity-stub enrichment, 429+ stubs) remains the parallel-friendly
fallback when no fetchable substantial work is ready.

\textbf{File carry-overs:} the \textit{Pro Milone} Latin slices and
English part files (\texttt{milone-latin-p1..p4}, \texttt{052bc-pro-milone-p1..p4})
were written to the Cowork \emph{outputs} scratch directory (outside
the repo), so they do NOT pollute the working tree --- no new in-repo
carry-overs this session. Earlier carry-overs persist (sandbox cannot
delete; Alexander handles at handoff): \textit{Att.} 5.1, 5.7
(\texttt{049bc-} $\rightarrow$ \texttt{051bc-}), \textit{Fam.} 13.09
(\texttt{005bc-} $\rightarrow$ \texttt{050bc-}), and the orphan
\texttt{054bc-ad-familiares-13-49} Latin.

\textbf{Launch-prompt advisory:} the scheduled 2-hour Cowork prompt
(``dispatch 3--4 parallel batches \ldots 3--5 short letters'') remains
stale and continues to auto-pivot. The project is in \textbf{Phase 3
(major-works sweep)}: one (or a few contiguous) substantial work(s) per
session, parallelized by section-range. The PM in each scheduled
session reads ``Where to resume now'' first and follows what it says,
so this is not blocking --- but updating the scheduled-task prompt to
Phase 3 wording is still recommended.

---

## Where to resume now (session 51 legacy snapshot below)

**Project management is now Claude Cowork** (running on Alexander's
desktop). When Cowork opens this repo, this is the first section to
read; \texttt{PLAN.md} is the binding multi-surface roadmap.

\textbf{Translation state (post-Cowork-session-51):} \textbf{923 /
958 works drafted (\~96.3\%)} --- session 51 continued \textbf{Phase 3
(the major-works sweep)} by clearing the three remaining 54 BC forensic
speeches in one parallel batch: \textit{Pro Plancio} (104 sections,
$\sim$12{,}900 English words), \textit{Pro Rabirio Postumo} (48
sections, $\sim$3{,}650 words), and the fragmentary \textit{Pro Scauro}
(76 surviving section-sigla, $\sim$2{,}930 words). These were the
chronologically-earliest non-deferred pending works; with them drafted,
the 54 BC tier of speeches is complete and the next non-deferred work
is \textit{Pro Milone} (52 BC).

\textbf{Method (parallel, mixed split):} the PM pre-fetched all three
via \texttt{scripts/fetch\_latin.py} (Perseus TEI
\texttt{phi028}/\texttt{phi030}/\texttt{phi029}) and dispatched six
worker sub-agents in one batch --- four splitting \textit{Pro Plancio}
by contiguous section-range (\textsection\textsection 1--26, 27--52,
53--78, 79--104) into part files under a shared register brief
(\texttt{056bc-pro-balbo} as the voice template), and one each
translating \textit{Pro Rabirio Postumo} and \textit{Pro Scauro}
end-to-end. The PM assembled the four \textit{Plancio} parts into
\texttt{english/speeches/054bc-pro-plancio.tex}, wrote that speech's
headnote itself (whole-speech understanding needed), flipped all three
statuses, and ran the consolidation scripts. Per-work parallel sidecars
were generated by \texttt{backfill\_structural.py} (104 / 48 / 76
segments, zero empties) rather than emitted by workers, to remove a
JSON-malformation risk; this is reliable because backfill aligns
mechanically on \texttt{\textbackslash ciceroSection} labels.

\textbf{Fragmentary-speech handling (\textit{Pro Scauro}):} the speech
survives only in fragments (Asconius and the grammarians) plus the
Ambrosian palimpsest, so its section labels are NOT a 1..N run but
editorial sigla: \texttt{a, c, d, e, f, g, h, i, k, m, n, o, p, 1q, 1r,
2s, 2--45, 45a--45n, 46o, 47--51} (\texttt{45e/45i/45j} are absent from
the source too). The English file reproduces these exact labels;
\texttt{validate.py} and the parallel-sidecar alignment confirm an
exact label-set match against the Latin.

\textbf{Headnote correction committed this session:} the \textit{Pro
Rabirio Postumo} headnote initially mis-stated Rabirius's parentage
(``natural son of Cicero's sister''); corrected to the accurate
genealogy --- son of the publicanus C. Curtius, adopted by the will of
his maternal uncle C. Rabirius (no relation to Cicero), as the speech
itself states at \textsection 3. \textbf{No date corrections}: these
are speeches with no Perseus datelines; the conventional 54 BC dating
is retained (\textit{Pro Rabirio Postumo} stays \texttt{year\_range}
-0054 to -0053) and all three files are correctly named \texttt{054bc-}.
\textbf{No Greek phrases} occur in any of the three.

\textbf{Translator-notes worth logging in
\texttt{data/translator-notes.jsonl}} (deferred to a future enrichment
pass to avoid concurrent-write contention):
\begin{itemize}
\item \textbf{\textit{Pro Plancio} \S 60} --- the centrepiece
sententia \textit{honorum gradus pares, gloriae dispares} rendered
``the steps of office are equal for the highest men and the lowest; the
steps of glory are unequal.''
\item \textbf{\textit{Pro Plancio} \S\S 36ff} --- \textit{editicii
iudices} rendered ``named jurors''; \textit{lex Licinia de sodaliciis}
glossed parenthetically on first occurrence, then kept Latin;
\textit{sodalitas}/\textit{sodales} = ``sodality''/``club-fellows.''
\item \textbf{\textit{Pro Rabirio Postumo}} --- the \textit{quo ea
pecunia pervenerit} clause rendered consistently ``to whom that money
came''; \textit{dioecetes} and \textit{pallium} kept as loaded
technical terms (glossary candidates).
\item \textbf{\textit{Pro Scauro}} --- damaged/broken fragments
(\texttt{n}; \texttt{51} = the single word \textit{universe}) rendered
as clean fragments with an ellipsis at the broken edge, not padded; the
Sardinian-witness invective (\textit{mastruca}, \textit{pelliti}) kept
literal.
\end{itemize}

\textbf{Validation:} \texttt{validate.py} clean --- 8 warnings = the
pre-existing orphan-Latin (\texttt{054bc-ad-familiares-13-49}) plus the
chronological-gap warning, now at \textbf{29} pending earlier-dated
works (down from 32 post-session-50, the expected $-3$ decrement since
all three speeches at 54 BC fell inside the chronological-gap zone).
\textbf{Latest drafted by deep chronology is unchanged}: \textit{Ad
Familiares} 10.24 (28 July 43 BC) remains the marker.
\texttt{build\_manifest.py} regenerated \texttt{MANIFEST.md} (958
entries) and \texttt{build/chronology.tex}.

\textbf{Suggested next translation batch} (when session 51 is landed
via \texttt{bash scripts/cowork\_handoff.sh "session 51: \ldots"} and
session 52 opens):

\textbf{Continue the Phase 3 major-works sweep in chronological
order.} The chronologically-next non-deferred pending work is
\textit{pro-milone} (52 BC, April --- a major speech, likely a
two-session split; use the parallel-by-section-range pattern, $\sim$28
sections per worker). Two manual-source snags follow immediately:
\textit{de-legibus} (52 BC, complete but not in Perseus --- needs
manual Latin) and \textit{ad-familiares-05-10} (51 BC --- absent from
Perseus \texttt{phi056}, Latin source TBD; skip or source manually).
After those the great 46--44 BC rhetorica/philosophica sequence opens:
\textit{brutus}, \textit{ad-familiares-09-25},
\textit{de-optimo-genere-oratorum}, \textit{paradoxa-stoicorum},
\textit{orator}, \textit{academica}, \textit{de-finibus},
\textit{tusculanae-disputationes}, \textit{de-natura-deorum},
\textit{de-officiis}, and so on. The long treatises in that run suit
the parallel-worker-by-section-range pattern used here and for
\textit{Partitiones}.
...
\textit{hortensius} (45), \textit{consolatio} (45) --- fragmentary;
plus the deliberately-skipped pre-\textit{Pro Quinctio} works
\textit{aratea} (86) and \textit{de-inventione} (85); and
\textit{ad-familiares-05-10} (51, Perseus-absent). \textbf{Slice M}
(entity-stub enrichment, 429+ stubs) remains the parallel-friendly
fallback when no fetchable substantial work is ready.

\textbf{File carry-overs:} the \textit{Pro Plancio} part files
\texttt{054bc-pro-plancio-p1..p4.tex} were written to the Cowork
\emph{outputs} scratch directory (outside the repo), so they do NOT
pollute the working tree --- no new in-repo carry-overs this session.
Earlier carry-overs persist (sandbox cannot delete; Alexander handles
at handoff): \textit{Att.} 5.1, 5.7 (\texttt{049bc-} $\rightarrow$
\texttt{051bc-}), \textit{Fam.} 13.09 (\texttt{005bc-} $\rightarrow$
\texttt{050bc-}), and the orphan \texttt{054bc-ad-familiares-13-49}
Latin.
...
## Where to resume now (session 50 legacy snapshot below)

**Project management is now Claude Cowork** (running on Alexander's
desktop). When Cowork opens this repo, this is the first section to
read; \texttt{PLAN.md} is the binding multi-surface roadmap.

\textbf{Translation state (post-Cowork-session-50):} \textbf{920 /
958 works drafted (\~96.0\%)} --- session 50 opened \textbf{Phase 3
(the major-works sweep)} by completing \textit{Partitiones Oratoriae}
(54 BC, 140 sections, $\sim$12{,}000 English words) end-to-end in one
session. This is the chronologically-earliest pending non-deferred
work and the first rhetorical treatise drafted since \textit{De
Oratore}. Latin fetched (70{,}214 chars, Perseus TEI
\texttt{phi0474.phi038}, a single self-contained dialogue).

\textbf{Method (parallel-by-section-range, new this session):} the PM
dispatched five worker sub-agents in parallel, each translating a
contiguous 28-section range (\textsection\textsection 1--28, 29--56,
57--84, 85--112, 113--140) into a separate part file under a single
shared locked-conventions brief (speaker tags, a fixed technical
glossary, and the STYLE.md voice); the PM then assembled the five
parts into the one \texttt{english/rhetoric/054bc-partitiones-oratoriae.tex},
wrote the three-paragraph headnote, flipped the status, and ran the
consolidation scripts. The catechism's dense stichomythic Q\&A is
front-loaded (\textsection\textsection 1--16, $\sim$66 speaker turns)
and tapers to near-continuous exposition by \textsection 85. Speaker
convention: the Latin labels the questioning son by the family name
(\texttt{C.}/\texttt{Cicero.}) against the answering father
(\texttt{P.}/\texttt{Pater.}); rendered \texttt{\textbackslash textsc\{Marcus.\}}
(son) and \texttt{\textbackslash textsc\{Cicero.\}} (father) for
clarity, with the choice explained in the headnote.

\textbf{Latin metadata correction committed during session 50:} the
Perseus TEI fetch misplaced the \textsection 86 milestone as a bare
inline ``86'' inside \textsection 85's text (``\ldots Qua re quoniam 86
in his versatur\ldots''). Restored a proper
\texttt{\textbackslash ciceroSection\{86\}} at the natural sentence
break (\textsection 86 begins ``Qua re quoniam in his versatur omne
consilium duobus\ldots''). The Latin file now carries all 140
contiguous sections (1--140, no gaps or dups), and the parallel
sidecar aligns 140 segments with no empties. \textbf{No date
correction}: the work's date is conventional and disputed (c. 54--46
BC); the meta entry's year-precision \texttt{-0054-07-01} is retained
and the file is correctly named \texttt{054bc-}. \textbf{No Greek
phrases} occur in the work; \texttt{greek-phrases.json} is unaffected.

\textbf{Translator-notes worth logging in
\texttt{data/translator-notes.jsonl}} (Cowork did not write the shared
jsonl directly, to avoid concurrent-write contention; deferred to a
future enrichment pass):
\begin{itemize}
\item \textbf{\textit{propositum} (\S\S 61--68)} rendered ``thesis''
(the abstract kernel of the indefinite question), a fresh choice
alongside the locked ``indefinite question / general inquiry.''
\item \textbf{\textit{expressa} (\S 6, \textit{oratio aut libera aut
expressa})} rendered ``extracted under pressure'' (testimony squeezed
out under duress), to avoid collision with \textit{elocutio} =
``expression.''
\item \textbf{\textit{praevaricatio} / \textit{praevaricator}
(\S\S 124--126)} rendered ``collusion'' / ``collusive defender,'' with
the \textit{varus} etymology kept literal.
\item \textbf{\textit{penus} / \textit{ruta caesa} (\S 107)} rendered
``the store'' / ``the fixtures and fittings'' (legal terms of art
quoted as definitional examples; candidates for a glossary note).
\item \textbf{\textit{ratio} / \textit{firmamentum} (\S 103)} rendered
``ground'' / ``buttress'' (a paired forensic opposition).
\item \textbf{\S 118} ``the conspiracy carried out in my consulship'' =
the Catilinarian conspiracy of 63 BC; kept literal per the
no-bracketed-notes rule, flagged for a possible glossary/crossref note.
\end{itemize}

\textbf{Validation:} \texttt{validate.py} clean --- 8 warnings = the
pre-existing orphan-Latin (\texttt{054bc-ad-familiares-13-49}) plus the
chronological-gap warning, now at \textbf{32} pending earlier-dated
works (down from 33 post-session-49, the expected $-1$ decrement, since
\textit{Partitiones} at 54 BC fell inside the chronological-gap zone).
\textbf{Latest drafted by deep chronology is unchanged}: \textit{Ad
Familiares} 10.24 (28 July 43 BC) remains the marker.

\textbf{Suggested next translation batch} (when session 50 is landed
via \texttt{bash scripts/cowork\_handoff.sh "session 50: \ldots"} and
session 51 opens):

\textbf{Continue the Phase 3 major-works sweep in chronological
order.} The chronologically-next pending non-deferred works after
\textit{Partitiones} (54 BC) are the three 54 BC speeches
\textit{pro-plancio}, \textit{pro-rabirio-postumo},
\textit{pro-scauro} (pro-scauro survives only in fragments), then
\textit{pro-milone} (52 BC, a major speech --- likely a two-session
split), then the letter \textit{ad-familiares-05-10} (51 BC), then the
great 46--44 BC sequence of rhetorica and philosophica:
\textit{brutus}, \textit{orator}, \textit{de-optimo-genere-oratorum},
\textit{paradoxa-stoicorum}, \textit{academica}, \textit{de-finibus},
\textit{timaeus}, \textit{de-natura-deorum},
\textit{tusculanae-disputationes}, \textit{cato-maior-de-senectute},
\textit{de-divinatione}, \textit{de-fato}, \textit{topica},
\textit{laelius-de-amicitia}, \textit{de-officiis}, and the 43 BC
\textit{Ad Brutum}/\textit{Ad Familiares} correspondence
(\textit{ad-brutum-01-16}, \textit{02-06}, \textit{02-07},
\textit{02-08}; \textit{ad-familiares-11-08}, \textit{11-17},
\textit{11-28}). Pick the chronologically-next pending substantial
work. \textbf{The parallel-worker-by-section-range pattern used for
Partitiones this session works well for the long treatises} (De
Finibus, Tusculans, De Natura Deorum, De Officiis, Academica): split
into $\sim$28-section ranges to part files under a single shared
locked-conventions brief, then PM assembles + writes the headnote +
runs \texttt{backfill\_structural.py}; very long treatises may still
take two sessions.

\textbf{No-Perseus / deferred works} remaining in the pending set (need
manual Latin sourcing --- do NOT \texttt{fetch\_latin.py} them):
\textit{de-legibus} (52 BC, complete, manual source needed),
\textit{de-consulatu-suo} (60), \textit{de-temporibus-suis} (54),
\textit{hortensius} (45), \textit{consolatio} (45) --- fragmentary;
plus the two deliberately-skipped pre-\textit{Pro Quinctio} works
\textit{aratea} (86) and \textit{de-inventione} (85). \textbf{Slice M}
(entity-stub enrichment, 429+ stubs) remains the parallel-friendly
fallback when no substantial work is ready to be picked up.

\textbf{File-rename carry-overs from earlier sessions} (sandbox cannot
delete files; Alexander handles at handoff): \textit{Att.} 5.1, 5.7
(\texttt{049bc-} $\rightarrow$ \texttt{051bc-}), \textit{Fam.} 13.09
(\texttt{005bc-} $\rightarrow$ \texttt{050bc-}), and the orphan
\texttt{054bc-ad-familiares-13-49} Latin. No new rename carry-overs in
session 50 (the \textit{Partitiones Oratoriae} files are correctly
named \texttt{054bc-}).

\textbf{Launch-prompt advisory for the scheduled 2-hour Cowork task on
Alexander's side:} the prompt text (\texttt{dispatch 3--4 parallel
batches of worker sub-agents, each translating 3--5 short letters
end-to-end}) remains stale and continues to auto-pivot. The bulk
letters and every speech except the genuine earlier-pending ones were
drafted long ago; the project is now in \textbf{Phase 3 (major-works
sweep)}: single-substantial-work-per-session, optionally parallelized
by section-range across worker sub-agents as done for
\textit{Partitiones} this session. Updating the scheduled-task prompt
to reflect Phase 3 of \texttt{OPERATING\_PLAN.md} is recommended but
not blocking --- the PM in each scheduled session reads ``Where to
resume now'' first and follows what it says.

---

## Where to resume now (session 49 legacy snapshot below)

**Project management is now Claude Cowork** (running on Alexander's
desktop). When Cowork opens this repo, this is the first section to
read; \texttt{PLAN.md} is the binding multi-surface roadmap.

\textbf{Translation state (post-Cowork-session-49):} \textbf{919 /
958 works drafted (\~95.9\%)} --- session 49 completed
\textit{Philippic 14} (21 April 43 BC, 38 sections, $\sim$5{,}400
English words) end-to-end in one session, in single-substantial-
speech mode. \textbf{This is the last of the Philippics and the last
surviving speech of Cicero; drafting it completes the Philippics
(1--14 all drafted).} Latin fetched (24{,}980 chars, Perseus TEI
\texttt{phi0474.phi035} selecting \texttt{speech\_index: '14'}, a
single speech and not a whole-book dump). Drafted in eight Edit
chunks (\textsection 1--5, 6--10, 11--15, 16--20, 21--25, 26--30,
31--35, 36--38). Three-paragraph headnote written (the speech answers
a motion to resume the toga after the victory at Forum Gallorum;
Cicero resists premature celebration since the war ends only with the
relief of Decimus Brutus at Mutina, then turns the supplicatio into an
implicit declaration of Antony as \textit{hostis} by moving an
unprecedented fifty-day thanksgiving and naming Hirtius, Pansa, and
the young Caesar \textit{imperatores}; \textsection 14--16 rebut the
rumour that Cicero meant to seize the fasces on the Parilia; the
peroration, \textsection 31--38, is the funeral oration for the fallen
of the Martian and Fourth legions and the formal motion for a
monument and rewards). Status flipped \texttt{pending}~$\rightarrow$~\texttt{drafted};
\texttt{backfill\_structural.py} regenerated the parallel sidecar
(38 segments,
\texttt{data/parallel/speeches/043bc-philippic-14.json}, $\sim$58KB,
verified section-aligned 1--38);
\texttt{build\_manifest.py} (958 entries) and \texttt{validate.py}
both run clean (8 warnings: pre-existing orphan-Latin for
\texttt{054bc-ad-familiares-13-49} plus the chronological-gap
warning, now at \textbf{33} pending earlier-dated works, down from 34
post-session-48, the expected $-1$ decrement). \textbf{Latest
drafted by deep chronology is unchanged}: \textit{Ad Familiares}
10.24 (28 July 43 BC) remains the marker, since \textit{Philippic
14} is dated 21 April 43 BC and so falls inside the chronological-gap
zone.

\textbf{No metadata corrections committed during session 49.} The
meta entry's day-precision date (\texttt{-0043-04-21}) is the
conventional date for the speech and is correct; the Philippics carry
no Perseus dateline. Perseus TEI numbering for Philippic 14 is regular
(no anomaly), and the fetch returned the single speech cleanly. The
\texttt{043bc-philippic-14} files are correctly named; no new rename
carry-overs in session 49.
...
\begin{itemize}
\item \textbf{\S 1--3 \textit{saga}/\textit{toga}.} The
war-cloak/civilian-dress antithesis drives the exordium: Rome had put
on the \textit{sagum} for the peril of Decimus Brutus, and the toga is
to be reserved for the completed victory. Rendered ``war-cloak'' for
\textit{sagum} and ``toga''/``civilian dress'' for
\textit{togati}/\textit{vestitus}, keeping the running opposition
without a footnote.
\item \textbf{\S 6 \textit{imbuti}\ldots\textit{madefacti}.} The
intensification kept: the swords ``have been dipped --- or rather
drenched.''
\item \textbf{\S 6 \textit{quo usque}\ldots\textit{hostis}.} The
deliberate echo of the Catilinarian \textit{quo usque tandem};
rendered naturally (``How long, then, shall the man who has outdone
all enemies in wickedness go without the name of enemy?'') without
flagging the allusion in the body.
\item \textbf{\S 14 fasces / Parilia.} The rumour that Cicero would
descend to the Forum with the fasces on the Parilia (21 April --- the
speech's own day) and make himself tyrant; the augural questions
(``By what auspices\ldots how long\ldots to whom hand them on?'')
preserved as a tricolon.
\item \textbf{\S 23 supplicatio precedents.} The Sulla/Cinna/Caesar
examples that no thanksgiving was ever decreed for victory in a civil
war drive the central argument: to decree one now is already to judge
Antony's men \textit{hostes}. ``Decreed, do I say? It was not even
asked for in the victor's dispatches.''
\item \textbf{\S 27 \textit{O solem ipsum beatissimum}.} The
apostrophe to the Sun who, before he set, saw Antony fleeing with his
few --- kept as direct apostrophe.
\item \textbf{\S 31 \textit{O fortunata mors}.} The funeral-oration
tag kept: ``O fortunate death, which, owed to nature, was paid above
all for the fatherland.''
\item \textbf{\S 32 Mars / Martia.} The etymological play preserved
--- the legion ``whose very name is from Mars,'' and ``Mars himself is
wont to claim as his own pledge the bravest from the battle-line.''
\item \textbf{\S 34 \textit{ara virtutis} (long period).} The
suspended consolation period rendered with its build intact to the
landing, ``an altar of valour\ldots unto the memory of eternity.''
\item \textbf{\S 36--38 the formal \textit{sententia}.} Decree
language rendered in a ``Whereas\ldots the Senate resolves
that\ldots'' register; \textit{alter ambove} = ``the one or both.''
\item \textbf{No Greek phrases} occur in this speech;
\texttt{greek-phrases.json} is unaffected by this work.
\end{itemize}

\textbf{Suggested next translation batch} (when session 49 is landed
via \texttt{bash scripts/cowork\_handoff.sh "session 49: \ldots"} and
session 50 opens):

\textbf{With the Philippics complete, the Phase 3 sweep of the major
works begins.} Session 50 should open the philosophical treatises and
remaining minor speeches as itemised in the session-46 snapshot below
(\textit{Cato Maior}, \textit{Paradoxa Stoicorum},
\textit{Academica}, \textit{De Finibus}, \textit{De Natura Deorum},
\textit{Tusculans}, \textit{De Officiis}, etc.), plus the genuine
earlier-pending works \textit{pro-plancio},
\textit{pro-rabirio-postumo}, \textit{pro-scauro},
\textit{partitiones-oratoriae}, and \textit{ad-familiares-05-10}, and
the 43 BC \textit{Ad Brutum}/\textit{Ad Familiares} correspondence
(note \textit{ad-brutum-01-01} is already drafted). Pick the
chronologically-next pending substantial work and translate it in
5-section chunks with WIP-emit discipline; long treatises may take two
sessions.

\textbf{Slice F (no-Perseus deferred letters)} and \textbf{Slice M
(entity-stub enrichment, 429+ stubs)} remain the parallel-friendly
fallbacks when no substantial work is ready to be picked up.
...
\texttt{051bc-}), \textit{Fam.} 13.09 (\texttt{005bc-}
$\rightarrow$ \texttt{050bc-}), and the orphan
\texttt{054bc-ad-familiares-13-49} Latin. No new rename carry-overs in
session 49 (the \textit{Philippic 14} files are correctly named
\texttt{043bc-}).
...
auto-pivot. Sessions 32--49 all dropped to single-substantial-
speech mode per this section's instructions; with the Philippics now
finished, the philosophical phase (sessions 50--62 or so) is likewise
single-substantial-work mode (5-section chunks, WIP-emit discipline,
multi-session splits allowed for very long treatises). Updating the
scheduled-task prompt to reflect Phase 3 of OPERATING\_PLAN.md is
recommended but not blocking --- the PM in each scheduled session
reads ``Where to resume now'' first and follows what it says.

---

## Where to resume now (session 48 legacy snapshot below)

**Project management is now Claude Cowork** (running on Alexander's
desktop). When Cowork opens this repo, this is the first section to
read; \texttt{PLAN.md} is the binding multi-surface roadmap.

\textbf{Translation state (post-Cowork-session-48):} \textbf{918 /
958 works drafted (\~95.8\%)} --- session 48 completed
\textit{Philippic 13} (20 March 43 BC, 50 sections, $\sim$8{,}000
English words) end-to-end in one session, in single-substantial-
speech mode. This is the longest of the Philippics; the session-47
snapshot had flagged it as a \emph{likely} two-session split, but it
was completed cleanly in a single session in ten Edit chunks
(\textsection 1--5, 6--10, 11--15, 16--20, 21--25, 26--30, 31--35,
36--40, 41--45, 46--50). Latin fetched (37{,}613 chars, Perseus TEI
\texttt{phi0474.phi035} selecting \texttt{speech\_index: '13'}, a
single speech and not a whole-book dump). Three-paragraph headnote
written (the speech answers a fresh peace overture --- M.~Aemilius
Lepidus's dispatch urging concord --- and turns Lepidus's own
authority against the peace he counsels; its centrepiece, \textsection
22--48, is the clause-by-clause reading and demolition of a letter
Antony had written to Hirtius and the young Caesar, forwarded to
Cicero by Hirtius; the first movement, \textsection 1--21, argues no
peace is possible with the Antonian faction and presses the case for
restoring the younger Magnus Pompeius; the peroration, \textsection
49--50, refuses all reconciliation and moves a decree commending
Pompeius). Status flipped \texttt{pending}~$\rightarrow$~\texttt{drafted};
\texttt{backfill\_structural.py} regenerated the parallel sidecar
(50 segments,
\texttt{data/parallel/speeches/043bc-philippic-13.json}, $\sim$89KB,
verified section-aligned 1--50);
\texttt{build\_manifest.py} (958 entries) and \texttt{validate.py}
both run clean (8 warnings: pre-existing orphan-Latin for
\texttt{054bc-ad-familiares-13-49} plus the chronological-gap
warning, now at \textbf{34} pending earlier-dated works, down from 35
post-session-47, the expected $-1$ decrement). \textbf{Latest
drafted by deep chronology is unchanged}: \textit{Ad Familiares}
10.24 (28 July 43 BC) remains the marker, since \textit{Philippic
13} is dated March 43 BC and so falls inside the chronological-gap
zone.

\textbf{No metadata corrections committed during session 48.} The
meta entry's day-precision date (\texttt{-0043-03-20}) is the
conventional date for the speech and is correct; the Philippics carry
no Perseus dateline. Perseus TEI numbering for Philippic 13 is regular
(no anomaly analogous to the Fam.\ 13.49 case), and the fetch returned
the single speech cleanly. The \texttt{043bc-philippic-13} files are
correctly named; no new rename carry-overs in session 48.
...
deferred to a future enrichment pass):
\begin{itemize}
\item \textbf{\S 17 \textit{maestitiam atque lacrimas}.} The
antecedent of \textit{eius} (and the subject of \textit{abiectus,
confectus esset}) is Antony, named in the \textit{cum}-clause: the
dejection and tears at the Lupercalia are Antony's own self-abasement
in offering Caesar the diadem (``would rather be that man's slave than
his colleague''), not the people's grief. Rendered ``his dejection and
his tears.''
\item \textbf{\S 22 \textit{O Spartace}.} Antony apostrophised as
``Spartacus''; the gladiator-slave insult, with Catiline made to seem
bearable by comparison.
\item \textbf{\S 24 \textit{mentum mentemque}.} The chin/mind pun
kept: ``you would lay your chin --- and your mind --- in the laps of
actresses.''
\item \textbf{\S 25 \textit{venefica}.} The feminine ``poisoner-woman''
hurled at Decimus Brutus; the \textit{venefica}/\textit{veneficia}
wordplay preserved (``do you dare call `poisoner' the man who has found
the remedies against your poisons?''). Also \textit{nove Hannibal} ---
``you new Hannibal.''
\item \textbf{\S 27 \textit{Muribus Deciis}.} The pun on the Decii
Mures (``the Mice'') and \textit{arrosit} (``gnawed at Caesar's
gifts'') --- mouse-imagery --- preserved.
\item \textbf{\S 35 \textit{parentasset} (crux).} A dense, compressed
counterfactual: ``Dolabella would have paid the death-offering to his
commander no sooner than Antony would have paid his to his colleague
as well.'' Rendered literally and flagged; the surrounding sentence is
heavily ironic.
\item \textbf{\S 40 \textit{lanista}.} ``Trainer of gladiators'';
\textit{unius corporis duas acies} = ``two battle-lines of one body''
(the body politic). Cicero accepts the label: ``I want the worse
slaughtered and the better to win.''
\item \textbf{\S 43 \textit{piissimus}.} The famous grammatical jab
rendered with its point intact and no footnote (per STYLE.md): a word
``that does not exist at all in the Latin tongue,'' which Antony, for
his ``divine piety,'' coins afresh.
\item \textbf{\S 49 \textit{prius undis flamma} / \textit{nec facta nec
nomen}.} Two poetic tags (``as some poet or other says'') kept in
quotation marks with the literal adynaton image (``sooner shall fire
mingle with the waves'').
\item \textbf{No Greek phrases} occur in this speech;
\texttt{greek-phrases.json} is unaffected by this work.
\end{itemize}

\textbf{Suggested next translation batch} (when session 48 is landed
via \texttt{bash scripts/cowork\_handoff.sh "session 48: \ldots"} and
session 49 opens):

\textbf{Session 49 opens \textit{Philippic 14} (21 April 43 BC,
$\sim$38 sections), the last of the series} --- delivered on news of
the victory at Mutina, proposing a public thanksgiving and honours for
the soldiers fallen in the battle. The meta date is day-precision
(\texttt{-0043-04-21}); no correction expected, and the fetch should
return a single speech via \texttt{speech\_index: '14'}. Drafting it
\textbf{completes the Philippics (1--14 all drafted).}

\textbf{After the Philippics} (sessions $\sim$50--62 or so): sweep
the major philosophical treatises and the remaining minor speeches
and rhetorica, exactly as itemised in the session-46 snapshot below
(\textit{Cato Maior}, \textit{Paradoxa Stoicorum},
\textit{Academica}, \textit{De Finibus}, \textit{De Natura Deorum},
\textit{Tusculans}, \textit{De Officiis}, etc.; plus the
earlier-pending \textit{pro-plancio}, \textit{pro-rabirio-postumo},
\textit{pro-scauro}, \textit{partitiones-oratoriae}, and
\textit{ad-familiares-05-10}, which are genuine pending works, not
dating errors).
...
$\rightarrow$ \texttt{050bc-}). No new rename carry-overs in
session 48 (the \textit{Philippic 13} files are correctly named
\texttt{043bc-}).
...
3--4 parallel batches of worker sub-agents, each translating
3--5 short letters end-to-end}) remains stale and continues to
auto-pivot. Sessions 32--48 all dropped to single-substantial-
speech mode per this section's instructions; the same auto-pivot
will continue through the last Philippic (session 49) and the
philosophical phase (sessions 50--62 or so). Updating the
scheduled-task prompt to reflect Phase 3 of OPERATING\_PLAN.md
(``one substantial-work worker per session, 5-section chunks,
WIP-emit discipline, multi-session splits allowed for very long
works'') is recommended but not blocking --- the PM in each
scheduled session reads ``Where to resume now'' first and follows
what it says.

---

## Where to resume now (session 47 legacy snapshot below)
...
\textbf{Translation state (post-Cowork-session-47):} \textbf{917 /
958 works drafted (\~95.7\%)} --- session 47 completed
\textit{Philippic 12} (March 43 BC, 30 sections, $\sim$5{,}100
English words) end-to-end in one session, in single-substantial-
speech mode. Latin fetched (21{,}967 chars, Perseus TEI
\texttt{phi0474.phi035} selecting \texttt{speech\_index: '12'}, a
single speech and not a whole-book dump), English translated in six
Edit chunks (\textsection 1--5, 6--10, 11--15, 16--20, 21--25,
26--30), three-paragraph headnote written (the speech of retraction:
Cicero withdraws both his assent to, and his own person from, the
proposed second peace embassy to Antony at Mutina; the two consulars
L.~Calpurnius Piso and Q.~Fufius Calenus, intimates of Antony, with
the consul Pansa lending weight, had revived the hope of an
``honourable peace'' and Cicero had let himself be named to the
mission; the speech argues that the embassy is folly and demoralizing
to towns, colonies, Italy, and the legions (the Martian and the
Fourth), that no peace is possible without unmaking the Senate's own
decrees --- the forged minutes, the laws carried by violence, the
seven hundred million sesterces embezzled, the offices and kingdoms
sold --- and dwells on Cicero's personal danger on the three roads to
Mutina (Flaminia, Aurelia, Cassia), closing conditionally: his life
to be kept for the commonwealth, the question measured by the public
good, and he will go only if it can be done safely). Status flipped
\texttt{pending}~$\rightarrow$~\texttt{drafted};
\texttt{backfill\_structural.py} regenerated the parallel sidecar
(30 segments,
\texttt{data/parallel/speeches/043bc-philippic-12.json}, $\sim$51KB);
\texttt{build\_manifest.py} (958 entries) and \texttt{validate.py}
both run clean (8 warnings: pre-existing orphan-Latin for
\texttt{054bc-ad-familiares-13-49} plus the chronological-gap
warning, now at 35 pending earlier-dated works, down from 36
post-session-46, the expected $-1$ decrement). \textbf{Latest
drafted by deep chronology is unchanged}: \textit{Ad Familiares}
10.24 (28 July 43 BC) remains the marker, since \textit{Philippic
12} is dated March 43 BC and so falls inside the chronological-gap
zone.

\textbf{No metadata corrections committed during session 47.} The
meta entry's month-precision date (\texttt{-0043-03-01}) was left as
is: the Philippics carry no Perseus dateline, and the conventional
placing is only ``early March 43 BC,'' so day-precision is not
warranted by the source and is not blocking. Perseus TEI numbering
for Philippic 12 is regular (no anomaly analogous to the Fam.\ 13.49
case), and the fetch returned the single speech cleanly (21{,}967
chars, not a whole-book dump). The \texttt{043bc-philippic-12} files
are correctly named; no new rename carry-overs in session 47.
...
deferred to a future enrichment pass):
\begin{itemize}
\item \textbf{\S 3, \S 5 \textit{caligo}.} The ``mist'' that ``had
dazzled the edge of my mind'' (\textit{praestrinxerat aciem animi},
\S 3) and is then ``scattered'' (\textit{discussa est illa caligo},
\S 5): the optical metaphor kept consistent across both sections.
\item \textbf{\S 5 \textit{cuiusvis hominis est errare; nullius nisi
insipientis perseverare in errore}.} Rendered fresh (``Any man may
err; none but a fool persists in his error'') rather than echoing the
proverbial ``to err is human.''
\item \textbf{\S 7 \textit{optimus est portus paenitenti mutatio
consili}.} ``The best harbour for a man who repents is a change of
plan'' --- the harbour image kept.
\item \textbf{\S 12 \textit{figentur} \ldots\ \textit{refixistis}.}
The bronze-tablet pun (the posted decrees fixed up / taken down):
``shall those tablets be fixed up again which you by your decrees took
down.''
\item \textbf{\S 12 \textit{eluatur} \ldots\ \textit{elui}.} Blood
``washed away'' vs.\ the renown of cruelty ``washed out'' --- the
wash pun preserved.
\item \textbf{\S 14 \textit{Tirones Numisios et mustelas Seios}.}
Contemptuous plural naming two of Antony's henchmen (Tiro and
Mustela); rendered ``such creatures as Numisius Tiro and Mustela
Seius.'' \textit{mustela} = ``weasel''; a possible pun on the cognomen
is flagged for the enrichment pass.
\item \textbf{\S 17 \textit{hostem} / \textit{adversarium};
\textit{bellum} / \textit{tumultum}.} The antitheses preserved (``an
enemy, when others called him an adversary; \ldots\ a war, when others
called it a tumult'').
\item \textbf{\S 20 \textit{ne aquilam quidem ipsam}.} The pun on
Aquila (the centurion's name) and \textit{aquila} (the legionary
eagle): ``not even the Eagle herself would have believed him.''
\item \textbf{\S 26 \textit{ne vino quidem permixta temperari}.} The
wine-tempering pun --- Antony's savagery not ``tempered'' even by an
admixture of wine --- kept on ``temper.''
\item \textbf{\S 27 \textit{voluntate hospitem, necessitate
hostem}.} Scato's reply, with the \textit{hospitem}/\textit{hostem}
near-pun: ``By goodwill a guest, by necessity a foe.''
\item \textbf{No Greek phrases} occur in this speech;
\texttt{greek-phrases.json} is unaffected by this work.
\end{itemize}

\textbf{Suggested next translation batch} (when session 47 is landed
via \texttt{bash scripts/cowork\_handoff.sh "session 47: \ldots"} and
session 48 opens):

\textbf{Session 48 opens \textit{Philippic 13} (20 March 43 BC,
$\sim$50 sections).} The longest of the remaining Philippics: Cicero
reads out Antony's letter to Hirtius and the young Caesar and answers
it clause by clause, and rejects any accommodation with the
Antonians. \textbf{Likely a two-session split} (e.g.\ \textsection
1--25 then \textsection 26--50), per the long-speech discipline in
\texttt{CLAUDE.md} (\textsection Timeout discipline: $\sim$5 sections
per Edit, WIP between chunks). The meta date is day-precision
(\texttt{-0043-03-20}); no correction expected.

\textbf{After \textit{Philippic 13}}: \textit{Philippic 14}
(21 April 43 BC, $\sim$38 sections, the last of the series, on the
victory at Mutina and honours for the fallen). Philippic phase after
this session: \textbf{2 speeches left (Phil 13--14; Phil 1--12 now
completed).}
...
\texttt{051bc-}) and \textit{Fam.} 13.09 (\texttt{005bc-}
$\rightarrow$ \texttt{050bc-}). No new rename carry-overs in
session 47 (the \textit{Philippic 12} files are correctly named
\texttt{043bc-}).
...
3--4 parallel batches of worker sub-agents, each translating
3--5 short letters end-to-end}) remains stale and continues to
auto-pivot. Sessions 32--47 all dropped to single-substantial-
speech mode per this section's instructions; the same auto-pivot
will continue through the last two Philippics and the philosophical
phase (sessions 48--62 or so). Updating the scheduled-task prompt to
reflect Phase 3 of OPERATING\_PLAN.md (``one substantial-work worker
per session, 5-section chunks, WIP-emit discipline, multi-session
splits allowed for very long works'') is recommended but not
blocking --- the PM in each scheduled session reads ``Where to
resume now'' first and follows what it says.

---

## Where to resume now (session 46 legacy snapshot below)
...
\textbf{Translation state (post-Cowork-session-46):} \textbf{916 /
958 works drafted (\~95.6\%)} --- session 46 completed
\textit{Philippic 11} (March 43 BC, 40 sections, $\sim$6{,}600
English words) end-to-end in one session: Latin fetched
(29{,}335 chars, Perseus TEI \texttt{phi0474.phi035} selecting
\texttt{speech\_index: '11'}, a single speech and not a whole-book
dump), English translated in single-substantial-speech mode in eight
Edit chunks (\textsection 1--5, 6--10, 11--15, 16--20, 21--25,
26--30, 31--35, 36--40), three-paragraph headnote written (the
speech on the torture and murder of C.~Trebonius --- proconsul of
Asia and one of Caesar's assassins --- by P.~Cornelius Dolabella at
Smyrna; the Senate having the day before declared Dolabella a public
enemy on Q.~Fufius Calenus's motion; the debate over who is to
command the war; Cicero's rejection of the two standing motions ---
L.~Caesar's extraordinary command for the \textit{privatus}
P.~Servilius Isauricus, and the assignment of Asia and Syria to the
consuls Pansa and Hirtius while D.~Brutus is besieged at Mutina; the
natural-law defence of Cassius's self-authorization
(\textsection 28, \textit{lex nihil aliud nisi recta ratio}); and
Cicero's own formal \textit{senatus consultum} (\textsection 29--31)
confirming C.~Cassius in Syria \textit{optimo iure}, taking over the
armies of Q.~Marcius Crispus, L.~Staius Murcus, and A.~Allienus,
with the kings Deiotarus father and son and the other client kings
invited to aid him). Status flipped
\texttt{pending}~$\rightarrow$~\texttt{drafted};
\texttt{backfill\_structural.py} regenerated the parallel sidecar
(40 segments,
\texttt{data/parallel/speeches/043bc-philippic-11.json}, $\sim$69KB);
\texttt{build\_manifest.py} and \texttt{validate.py} both run clean
(8 warnings: pre-existing orphan-Latin for
\texttt{054bc-ad-familiares-13-49} plus the chronological-gap
warning, now at 36 pending earlier-dated works, down from 37
post-session-45, the expected $-1$ decrement). \textbf{Latest
drafted by deep chronology is unchanged}: \textit{Ad Familiares}
10.24 (28 July 43 BC) remains the marker, since \textit{Philippic
11} is dated March 43 BC and so falls inside the chronological-gap
zone.

\textbf{No metadata corrections committed during session 46.} The
meta entry's month-precision date (\texttt{-0043-03-01}) was left as
is: the Philippics carry no Perseus dateline, and the conventional
placing is only ``late February / March 43 BC,'' so day-precision is
not warranted by the source and is not blocking. Perseus TEI
numbering for Philippic 11 is regular (no anomaly analogous to the
Fam.\ 13.49 case), and the fetch returned the single speech cleanly
(29{,}335 chars, not a whole-book dump). The
\texttt{043bc-philippic-11} files are correctly named; no new rename
carry-overs in session 46.
...
deferred to a future enrichment pass):
\begin{itemize}
\item \textbf{\S 14 \textit{novarum tabularum novam tabulam}.} Pun
on \textit{novae tabulae} (cancellation of debts) and \textit{tabula}
(an auction / bankruptcy posting): Trebellius, champion of
debt-cancellation, ``turned into a new account-book himself.''
\item \textbf{\S 14 \textit{Lysidicum ipsum \ldots\ iure Germanum
Cimber occidit}.} Double pun: Annius Cimber's father was named
Lysidicus (Greek ``loosener of suits''), so Cicero calls Cimber
``Lysidicus himself'' for dissolving every law; \textit{Germanum} =
both ``his blood brother'' and ``a German,'' and the cognomen Cimber
names a Germanic tribe, so a Cimbrian killing a German is darkly
``by right.'' Rendered ``unless perhaps it was by right that the
Cimbrian killed his own brother, a German.''
\item \textbf{\S 17 \textit{populare atque ventosum}.} Of
extraordinary commands; rendered ``a popular, windy thing,'' keeping
the wind image rather than flattening to ``fickle.''
\item \textbf{\S 18 \textit{pro consulibus \ldots\ non pro
consule}.} The L.~Philippus pun: \textit{pro consulibus} (in place
of the two consuls, who refused) vs.\ \textit{pro consule} (as
proconsul). Rendered ``in place of the consuls --- not as a
proconsul.''
\item \textbf{\S 24 \textit{id signum quod de caelo delapsum Vestae
custodiis continetur}.} The Palladium (image of Pallas fallen from
heaven, kept in Vesta's temple); rendered descriptively, the proper
name kept out of the body per the no-body-notes rule.
\item \textbf{\S 26 \textit{solet ipse accipere manicas}.} Of
C.~Antonius; rendered ``he is wont to hold out his own wrists for the
manacles'' --- the jibe that he surrenders rather than withstand a
siege.
\item \textbf{\S 28 \textit{lex nihil aliud nisi recta et a numine
deorum tracta ratio}.} The natural-law definition, rendered freshly
(``law is nothing else than right reason, drawn from the will of the
gods, commanding what is honourable and forbidding its opposite'')
rather than echoing the familiar \textit{De Re Publica} 3 /
Lactantius phrasings.
\item \textbf{\S 35 \textit{maximam eius et singularem laudem
praetermitto}.} The \textit{praeteritio} of Cassius's part in
Caesar's assassination (``not yet welcome to all''), preserved as
such.
\item \textbf{\S 5, \S 7 \textit{Samiarius}.} The executioner to
whom Trebonius was handed: rendered ``a Samian exile'' (\S 5) and
``the Samian torturer and executioner'' (\S 7), kept consistent. The
word may instead mean ``armourer / sword-polisher''; flagged for the
enrichment pass.
\item \textbf{No Greek phrases} occur in this speech;
\texttt{greek-phrases.json} is unaffected by this work.
\end{itemize}

\textbf{Suggested next translation batch} (when session 46 is landed
via \texttt{bash scripts/cowork\_handoff.sh "session 46: \ldots"} and
session 47 opens):

\textbf{Session 47 opens \textit{Philippic 12} ($\sim$March 43 BC,
$\sim$30 sections).} The speech in which Cicero, having at first
agreed to serve, withdraws from the proposed peace embassy to Antony
and argues against negotiating with him --- a moderate-length
deliberative speech, comfortably a single session at $\sim$6 chunks
of 5 sections. The meta date is a month-precision placeholder
(\texttt{-0043-03-01}); conventionally placed in March 43 BC.

\textbf{After \textit{Philippic 12}} (sessions 48--49 or so):
\textit{13} (20 Mar 43, $\sim$50 sections --- likely a two-session
split, e.g.\ \textsection 1--25 then \textsection 26--50) and
\textit{14} (21 Apr 43, $\sim$38 sections, the last of the series, on
the victory at Mutina and honours for the fallen). Philippic phase
after this session: 3 speeches left (Phil 12--14; Phil 1--11 now
completed).

\textbf{After the Philippics} (sessions $\sim$50--62 or so): sweep
the major philosophical treatises (\textit{Cato Maior De
Senectute}, \textit{Paradoxa Stoicorum}, \textit{Academica},
\textit{De Finibus}, \textit{Timaeus}, \textit{De Natura Deorum},
\textit{Tusculanae Disputationes}, \textit{De Divinatione},
\textit{De Fato}, \textit{Laelius De Amicitia}, \textit{De
Officiis}). The remaining minor speeches and the rhetorica
(\textit{partitiones-oratoriae}, \textit{brutus},
\textit{de-optimo-genere-oratorum}, \textit{orator},
\textit{topica}) are slotted alongside as one-session pickups; note
that the validator's earlier-pending list still surfaces
\textit{pro-plancio}, \textit{pro-rabirio-postumo},
\textit{pro-scauro}, and \textit{partitiones-oratoriae} (54 BC) and
\textit{ad-familiares-05-10} (51 BC) --- these are genuine pending
works, not dating errors.

\textbf{Slice F (the no-Perseus deferred letters)} continues to
...
\texttt{051bc-}) and \textit{Fam.} 13.09 (\texttt{005bc-}
$\rightarrow$ \texttt{050bc-}). No new rename carry-overs in
session 46 (the \textit{Philippic 11} files are correctly named
\texttt{043bc-}).
...
3--4 parallel batches of worker sub-agents, each translating
3--5 short letters end-to-end}) remains stale and continues to
auto-pivot. Sessions 32--46 all dropped to single-substantial-
speech mode per this section's instructions; the same auto-pivot
will continue through the remaining Philippic and philosophical
phases (sessions 47--62 or so). Updating the scheduled-task prompt
to reflect Phase 3 of OPERATING\_PLAN.md (``one substantial-work
worker per session, 5-section chunks, WIP-emit discipline,
multi-session splits allowed for very long works'') is recommended
but not blocking --- the PM in each scheduled session reads
``Where to resume now'' first and follows what it says.

---

## Where to resume now (session 45 legacy snapshot below)
...
\textbf{Translation state (post-Cowork-session-45):} \textbf{915 /
958 works drafted (\~95.5\%)} --- session 45 completed
\textit{Philippic 10} (February 43 BC, 26 sections,
$\sim$4{,}316 English words) end-to-end in one session: Latin
fetched (19{,}096 chars, Perseus TEI \texttt{phi0474.phi035}
selecting \texttt{speech\_index: '10'}, a single speech and not a
whole-book dump), English translated in single-substantial-speech
mode in five Edit chunks (\textsection 1--5, 6--10, 11--15,
16--20, 21--26), three-paragraph headnote written (the speech of
confirmation seconding the consul Pansa and answering Q.\ Fufius
Calenus, delivered after M. Brutus's despatch reported that he had
brought Macedonia, Illyricum, and all Greece into the power of the
Senate and Roman people; the governing principle that all legions
belong to the commonwealth and that a man forfeits army and command
the moment he turns them on the state; the answer to the
veterans-distrust objection by widening the ground beyond the
veterans alone; the death-before-slavery creed of
\textsection 19--20 (\textit{non in spiritu vita est, sed ea nulla
est omnino servienti}); the muster of Brutus's lieutenants ---
Q.~Hortensius, the younger Cicero, Cn.~Domitius, M.~Apuleius, P.~Vatinius; and the
closing \textit{senatus consultum} of \textsection 25--26
confirming Brutus, styled by his adoptive name \textit{Q.\ Caepio
Brutus pro consule}, in command of Macedonia, Illyricum, and Greece
with power to raise money, contract loans, requisition grain, and
keep his forces near Italy, and continuing Hortensius in the
province until a successor is named). Status flipped
\texttt{pending}~$\rightarrow$~\texttt{drafted};
\texttt{backfill\_structural.py} regenerated the parallel sidecar
(26 segments,
\texttt{data/parallel/speeches/043bc-philippic-10.json}, $\sim$45KB);
\texttt{build\_manifest.py} and \texttt{validate.py} both run clean
(8 warnings: pre-existing orphan-Latin for
\texttt{054bc-ad-familiares-13-49} plus the chronological-gap
warning, now at 37 pending earlier-dated works, down from 38
post-session-44, the expected $-1$ decrement). \textbf{Latest
drafted by deep chronology is unchanged}: \textit{Ad Familiares}
10.24 (28 July 43 BC) remains the marker, since \textit{Philippic
10} is dated February 43 BC and so falls inside the
chronological-gap zone.

\textbf{No metadata corrections committed during session 45.} The
meta entry's month-precision date (\texttt{-0043-02-01}) was left
as is: the Philippics, unlike the letters, carry no Perseus
dateline, and the conventional placing is only ``February 43 BC,''
so day-precision was not warranted by the source and is not
blocking. Perseus TEI numbering for Philippic 10 is regular (no
anomaly analogous to the Fam.\ 13.49 case), and the fetch returned
the single speech cleanly (19{,}096 chars, not a whole-book dump).
The \texttt{043bc-philippic-10} files are correctly named; no new
rename carry-overs in session 45.
...
deferred to a future enrichment pass):
\begin{itemize}
\item \textbf{\S 5 \textit{di meliora!}} rendered ``The gods
forbid!'' --- keeping the Roman plural gods (per STYLE.md's
no-Christianizing rule) rather than ``God forbid!''; it is Calenus's
imagined reply when asked whether he would wish his son like Brutus
or like an Antonius.
\item \textbf{\S 5--6 the clerk jibe.} \textit{librarium Bruti
laudare, non Brutum} rendered ``to praise Brutus's clerk, and not
Brutus''; Calenus's praise of the letter as \textit{recte et ordine
scriptas} quoted as ``well and properly written,'' tied to the
\S 6 mockery that the Senate has never decreed a letter ``well
written.''
\item \textbf{\S 11 \textit{caducae hereditates}} rendered
``windfall inheritances'' (the lapsed estates C.~Antonius stopped to
plunder on the road), preserving the joke that otherwise ``you would
say he had flown, not made a journey.''
\item \textbf{\S 17--18 section split at \textit{quae, malum!}} The
Latin numbering breaks the sentence \textit{quae, malum, est ista
ratio\ldots} across the section boundary; the English break is
placed inside it too (\S 17 ends ``What, confound it ---'', \S 18
begins ``is this the method\ldots''), preserving the Latin grain.
\textit{malum} as interjection rendered ``confound it.''
\item \textbf{\S 19--20 the death-before-slavery creed.}
\textit{non enim in spiritu vita est, sed ea nulla est omnino
servienti} rendered ``life does not consist in breath; rather,
there is none at all for one who is a slave''; \textit{Martem
communem} rendered ``Mars belongs to both sides alike'' (Roman form
kept, not flattened to ``the chance of war'').
\item \textbf{\S 24--25 and \S 25--26 section splits across names.}
As in the Philippic 9 treatment, the breaks fall inside the names
--- \S 24 ends ``on the matter that Gaius''~\textbar~\S 25 ``Pansa
the consul\ldots'', and \S 25 ends ``and that Quintus''~\textbar~%
\S 26 ``Caepio Brutus, proconsul\ldots'' --- preserving the Latin
section grain inside the quoted \textit{senatus consultum}.
\item \textbf{\S 22 \textit{cum tabulis novis redit in gratiam}}
rendered ``now, with a cancellation of debts, comes back into
favour''; \textit{Saxae et Cafones} (contemptuous plurals of
Antony's henchmen Saxa and Cafo) rendered ``the Saxas and the
Cafos.''
\item \textbf{No Greek phrases} occur in this speech;
\texttt{greek-phrases.json} is unaffected by this work.
\end{itemize}

\textbf{Suggested next translation batch} (when session 45 is
landed via \texttt{bash scripts/cowork\_handoff.sh "session 45:
\ldots"} and session 46 opens):

\textbf{Session 46 opens \textit{Philippic 11} ($\sim$March 43 BC,
$\sim$40 sections).} The speech on the torture and murder of
C.~Trebonius by Dolabella in Asia, and the proposal to declare
Dolabella a public enemy and assign the command against him --- a
long speech near the upper bound for a single session, so a
two-session split is plausible (e.g.\ \textsection 1--22, then
\textsection 23--40). Plan for seven or eight 5-section Edit chunks
with WIP-emit discipline. The meta date is a month-precision
placeholder (\texttt{-0043-03-01}); the speech is conventionally
placed in late February / March 43 BC.

\textbf{After \textit{Philippic 11}} (sessions 47--49 or so):
\textit{12} (Mar 43, $\sim$30 sections), \textit{13} (20 Mar 43,
$\sim$50 sections --- likely a two-session split), \textit{14}
(21 Apr 43, $\sim$38 sections). Philippic phase after this session:
4 speeches left (Phil 11--14; Phil 3--10 now completed).
...
\texttt{051bc-}) and \textit{Fam.} 13.09 (\texttt{005bc-}
$\rightarrow$ \texttt{050bc-}). No new rename carry-overs in
session 45 (the \textit{Philippic 10} files are correctly named
\texttt{043bc-}).
...
3--4 parallel batches of worker sub-agents, each translating
3--5 short letters end-to-end}) remains stale and continues to
auto-pivot. Sessions 32--45 all dropped to single-substantial-
speech mode per this section's instructions; the same auto-pivot
will continue through the remaining Philippic and philosophical
phases (sessions 46--62 or so). Updating the scheduled-task prompt
to reflect Phase 3 of OPERATING\_PLAN.md (``one substantial-work
worker per session, 5-section chunks, WIP-emit discipline,
multi-session splits allowed for very long works'') is recommended
but not blocking --- the PM in each scheduled session reads
...
## Where to resume now (session 44 legacy snapshot below)
...
\textbf{Translation state (post-Cowork-session-44):} \textbf{914 /
958 works drafted (\~95.4\%)} --- session 44 completed
\textit{Philippic 9} (4 February 43 BC, 17 sections,
$\sim$2{,}639 English words) end-to-end in one session: Latin
fetched (11{,}721 chars, Perseus TEI \texttt{phi0474.phi035}
selecting \texttt{speech\_index: '9'}, a single speech and not a
whole-book dump), English translated in single-substantial-speech
mode in four Edit chunks (\textsection 1--5, 6--10, 11--15,
16--17), three-paragraph headnote written (the funeral
\textit{laudatio} for Servius Sulpicius Rufus --- consul 51 BC and
the foremost jurist of the age, who died on the embassy to Antony
at Mutina; the speech answers P. Servilius Vatia Isauricus, who
would grant a statue only to an envoy killed by the sword, with the
argument that the ancestors looked to the \textit{cause} of death
and not its \textit{manner}; the \textit{exempla} of the four
envoys slain by Lars Tolumnius at Fidenae and of Cn. Octavius
murdered at Laodicea, both given statues on the Rostra; the
connoisseur's choice of a bronze \textit{pedestrian} statue over a
gilded equestrian one, in keeping with Sulpicius's love of
old-Roman restraint; and the closing \textit{senatus consultum} of
\textsection 15--17 --- statue, reserved family space at the games,
inscription, public funeral, and a public tomb in the Esquiline
field). Status flipped \texttt{pending}~$\rightarrow$~\texttt{drafted};
\texttt{backfill\_structural.py} regenerated the parallel sidecar
(17 segments,
\texttt{data/parallel/speeches/043bc-philippic-9.json}, $\sim$27KB);
\texttt{build\_manifest.py} and \texttt{validate.py} both run clean
(8 warnings: pre-existing orphan-Latin for
\texttt{054bc-ad-familiares-13-49} plus the chronological-gap
warning, now at 38 pending earlier-dated works, down from 39
post-session-43, the expected $-1$ decrement). \textbf{Latest
drafted by deep chronology is unchanged}: \textit{Ad Familiares}
10.24 (28 July 43 BC) remains the marker, since \textit{Philippic
9} is dated 4 February 43 BC and so falls inside the
chronological-gap zone.

\textbf{No metadata corrections committed during session 44.} The
meta entry was already day-precision and correct; Perseus TEI
numbering for Philippic 9 is regular (no anomaly analogous to the
Fam.\ 13.49 case), and the fetch returned the single speech cleanly
(11{,}721 chars, not a whole-book dump).
...
deferred to a future enrichment pass):
\begin{itemize}
\item \textbf{\S 3 the legal pivot.} \textit{causam mortis
censuerint, non genus esse quaerendum} rendered ``they held the
cause of death, not its manner, was the thing to be looked to'' ---
\textit{genus} as ``manner'' rather than ``kind/type,'' since the
antithesis is cause-versus-mode-of-death, the hinge of the whole
argument against P. Servilius.
\item \textbf{\S 13 the statue.} \textit{aenea statua \ldots et ea
pedestris quam inaurata equestris} rendered ``a bronze statue, and
that one on foot \ldots than a gilded equestrian one'';
\textit{pedestris} kept as ``on foot''/``pedestrian'' to preserve
the point about old-Roman restraint, with the Latin word surfaced
once in the headnote. \textit{tamquam ex eius auctoritate et
voluntate} $\to$ ``as though by his own authority and will.''
\item \textbf{\S 13--14 section split across a name.} The Latin
numbering breaks \textit{P.}~\textbar~\textit{Servili} mid-name;
the English section break is placed so it also falls inside the
name (``the proposal of Publius''~\textbar~``Servilius''),
preserving the Latin grain rather than smoothing it over.
\item \textbf{\S 15--16 section split inside the decree.} Same
treatment: the break falls across \textit{Ser.}~\textbar~%
\textit{Sulpicius} (``in which Servius''~\textbar~``Sulpicius was
often of great service'') inside the quoted \textit{senatus
consultum}.
\item \textbf{\S 15--17 the \textit{senatus consultum}.} The decree
rendered inside `` '' quotation marks; the tribal formula
\textit{Q. f. lemonia Rufus} rendered ``son of Quintus, of the
Lemonian tribe, Rufus''; \textit{uti quod optimo iure publice
sepulcrum datum esset} $\to$ ``under the same right as a tomb
granted at public expense under the best title.''
\item \textbf{No Greek phrases} occur in this speech;
\texttt{greek-phrases.json} is unaffected by this work.
\end{itemize}

\textbf{Suggested next translation batch} (when session 44 is
landed via \texttt{bash scripts/cowork\_handoff.sh "session 44:
\ldots"} and session 45 opens):

\textbf{Session 45 opens \textit{Philippic 10} ($\sim$February 43
BC, $\sim$26 sections).} The speech in support of Marcus Brutus,
confirming his command over Macedonia, Illyricum, and Greece,
delivered after Brutus's despatch reporting that he held the East
--- another running duel with Q. Fufius Calenus. At $\sim$26
sections it fits a single session, five or six 5-section Edit
chunks. The meta date is a month-precision placeholder
(\texttt{-0043-02-01}); the speech is conventionally placed in
mid/late February 43 BC, so the next session may refine the
dateline to day-precision if a firmer date is wanted (not
blocking).

\textbf{After \textit{Philippic 10}} (sessions 46--49 or so):
\textit{11} (Mar 43, $\sim$40 sections --- nearer the upper bound,
possibly a two-session split), \textit{12} (Mar 43, $\sim$30
sections), \textit{13} (20 Mar 43, $\sim$50 sections --- likely a
two-session split), \textit{14} (21 Apr 43, $\sim$38 sections).
Philippic phase after this session: 5 speeches left (Phil 10--14;
Phil 3--9 now completed).
...
\texttt{051bc-}) and \textit{Fam.} 13.09 (\texttt{005bc-}
$\rightarrow$ \texttt{050bc-}). No new rename carry-overs in
session 44 (the \textit{Philippic 9} files are correctly named
\texttt{043bc-}).
...
3--4 parallel batches of worker sub-agents, each translating
3--5 short letters end-to-end}) remains stale and continues to
auto-pivot. Sessions 32--44 all dropped to single-substantial-
speech mode per this section's instructions; the same auto-pivot
will continue through the remaining Philippic and philosophical
phases (sessions 45--62 or so). Updating the scheduled-task prompt
to reflect Phase 3 of OPERATING\_PLAN.md (``one substantial-work
worker per session, 5-section chunks, WIP-emit discipline,
multi-session splits allowed for very long works'') is recommended
but not blocking --- the PM in each scheduled session reads
...
## Where to resume now (session 43 legacy snapshot below)
...
\textbf{Translation state (post-Cowork-session-43):} \textbf{913 /
958 works drafted (\~95.3\%)} --- session 43 completed
\textit{Philippic 8} (3 February 43 BC, 33 sections,
$\sim$4{,}773 English words) end-to-end in one session: Latin
fetched (20{,}923 chars, Perseus TEI \texttt{phi0474.phi035}
selecting \texttt{speech\_index: '8'}), English translated in
single-substantial-speech mode in seven Edit chunks
(\textsection 1--5, 6--10, 11--15, 16--20, 21--25, 26--30,
31--33), four-paragraph headnote written (composition history ---
the senatorial \textit{tumultus} speech delivered the morning
after the legation returned from Antony at Mutina, reopening the
question lost the previous day when, under Pansa's presidency, the
Senate had decreed a \textit{tumultus} and the war-cloaks on
Lucius Caesar's milder motion but balked at the word
\textit{bellum} --- and the speech's structure: the lexical
argument that there can be war without tumult but no tumult
without war (\textsection 1--4), the proof that this is a war
whatever it is called (\textsection 5--8), the antithesis of
Antony's promises and the Senate's (\textsection 9--10), the
named duel with Q. Fufius Calenus over whether slavery can be
called peace (\textsection 11--19, with the \textit{exempla} of
justified senatorial force --- Nasica, Opimius, Marius, Catiline
--- and the surgeon's metaphor of the amputated limb), the
indictment of the second embassy and Antony's ``modest''
counter-demands (\textsection 20--28, with the Popilius Laenas
line-in-the-sand \textit{exemplum}), the digression on the duty
of a princeps and the example of the aged augur Scaevola
(\textsection 29--32), and the formal motion of
amnesty-before-the-Ides-of-March (\textsection 33)). Status
flipped \texttt{pending}~$\rightarrow$~\texttt{drafted};
\texttt{backfill\_structural.py} regenerated the parallel sidecar
(33 segments,
\texttt{data/parallel/speeches/043bc-philippic-8.json},
$\sim$49KB); \texttt{build\_manifest.py} and \texttt{validate.py}
both run clean (8 warnings: pre-existing orphan-Latin for
\texttt{054bc-ad-familiares-13-49} plus the chronological-gap
warning, now at 39 pending earlier-dated works, down from 40
post-session-42, the expected $-1$ decrement). \textbf{Latest
drafted by deep chronology is unchanged}: \textit{Ad Familiares}
10.24 (28 July 43 BC) remains the marker, since \textit{Philippic
8} is dated 3 February 43 BC and so falls inside the
chronological-gap zone.

\textbf{No metadata corrections committed during session 43.} The
meta entry was already correct, day-precision; Perseus TEI
numbering for Philippic 8 is regular (no anomaly analogous to the
Fam.\ 13.49 case), and the fetch returned the single speech
cleanly (20{,}923 chars, not a whole-book dump).
...
deferred to a future enrichment pass):
\begin{itemize}
\item \textbf{\S 2--4 the \textit{bellum}/\textit{tumultus}
distinction.} \textit{potest enim esse bellum ut tumultus non
sit, tumultus autem esse sine bello non potest} rendered ``there
can be a war such that there is no tumult; but a tumult cannot
exist without a war.'' \textit{Tumultus} kept as the cognate
``tumult'' throughout, since the whole argument turns on the word
itself; the folk-etymology behind \textit{unde etiam nomen ductum
est} (from \textit{timor multus}) left implicit (``from which the
very name `tumult' is derived''), per STYLE.md's bar on
footnoting in the body.
\item \textbf{\S 5 \textit{Gallia vastatur: quae pax potest esse
certior?}} Rendered with an exclamation --- ``Gaul is being laid
waste: what peace could be more secure!'' --- rather than a
question, so the sarcasm lands in English.
\item \textbf{\S 9 \textit{di meliora!}} Rendered ``May the gods
grant us better!'', not ``God forbid,'' per STYLE.md's
no-Christianizing rule.
\item \textbf{\S 9 \textit{hasta Caesaris}.} Rendered ``Caesar's
auction-spear'' (the spear set up to mark a public auction of
confiscated property), keeping the Roman image; the recurring
\textit{hastam videre cupiunt} rendered ``longing to see the
spear.''
\item \textbf{\S 23 \textit{virgula stantem circumscripsit}.} The
Popilius Laenas line-in-the-sand: ``with a little rod drew a
circle around him as he stood''; \textit{senatus faciem secum
attulerat} $\to$ ``he had brought with him the very face of the
Senate.''
\item \textbf{\S 26--27 the demands catalogue.} \textit{rationes
ad Opis} / \textit{septiens miliens} rendered ``the accounts at
the temple of Ops'' / ``the seven hundred million'';
\textit{vii viris} kept as ``the Septemviri''; \textit{Galliam
togatam $\ldots$ comatam} rendered ``the toga-clad Gaul $\ldots$
the long-haired,'' preserving the Cisalpine/Transalpine wordplay.
\item \textbf{\S 32 \textit{cum est in sagis civitas}.} The
consular's privilege of keeping the toga during a
\textit{tumultus} rendered ``even when the state is in
war-cloaks''; Cicero announces he waives it in solidarity.
\end{itemize}

\textbf{Suggested next translation batch} (when session 43 is
landed via \texttt{bash scripts/cowork\_handoff.sh "session 43:
\ldots"} and session 44 opens):

\textbf{Session 44 opens \textit{Philippic 9} (4 February 43 BC,
$\sim$17 sections).} The funeral speech on the death of Servius
Sulpicius Rufus --- the great jurist and legate who died on the
embassy to Antony --- proposing honours for him (a bronze statue
on the Rostra and a public tomb). At $\sim$17 sections it fits a
single session comfortably, three or four 5-section Edit chunks.

\textbf{After \textit{Philippic 9}} (sessions 45--49 or so):
\textit{10} (Feb 43, $\sim$26 sections), \textit{11} (Mar 43,
$\sim$40 sections), \textit{12} (Mar 43, $\sim$30 sections),
\textit{13} (20 Mar 43, $\sim$50 sections), \textit{14} (21 Apr
43, $\sim$38 sections). Each fits a single session (with the
longer two --- 11 and 13 --- nearer the upper bound). Total
Philippic phase is now 6 sessions left (Phil 3--8 completed; Phil
9--14 to go).

\textbf{After the Philippics} (sessions $\sim$50--60 or so): sweep
the major philosophical treatises (\textit{Cato Maior De
Senectute}, \textit{Paradoxa Stoicorum}, \textit{Academica},
\textit{De Finibus}, \textit{Timaeus}, \textit{De Natura Deorum},
\textit{Tusculanae Disputationes}, \textit{De Divinatione},
...
to require a single-session manual-fetch batch and is held until
someone supplies the Latin from a standard printed edition.
...
\texttt{051bc-}) and \textit{Fam.} 13.09 (\texttt{005bc-}
$\rightarrow$ \texttt{050bc-}). No new rename carry-overs in
session 43 (the \textit{Philippic 8} files are correctly named
\texttt{043bc-}).
...
3--4 parallel batches of worker sub-agents, each translating
3--5 short letters end-to-end}) remains stale and continues to
auto-pivot. Sessions 32--43 all dropped to single-substantial-
speech mode per this section's instructions; the same auto-pivot
will continue through the remaining Philippic and philosophical
phases (sessions 44--60 or so). Updating the scheduled-task prompt
to reflect Phase 3 of OPERATING\_PLAN.md (``one substantial-work
worker per session, 5-section chunks, WIP-emit discipline,
multi-session splits allowed for very long works'') is recommended
but not blocking --- the PM in each scheduled session reads
...
## Where to resume now (session 42 legacy snapshot below)

\textbf{Translation state (post-Cowork-session-42):} \textbf{912 /
958 works drafted (\~95.2\%)} --- session 42 completed
\textit{Philippic 7} (mid-January 43 BC, 27 sections,
$\sim$4{,}050 English words) end-to-end in one session: Latin
fetched (16{,}031 chars, Perseus TEI \texttt{phi0474.phi035}
selecting \texttt{speech\_index: '7'}), English translated in
single-substantial-speech mode in six Edit chunks
(\textsection 1--5, 6--10, 11--15, 16--20, 21--25, 26--27),
four-paragraph headnote written (composition history --- the
holding speech of the legation interval, delivered in the
Senate in mid-January 43 BC while the legates Sulpicius Rufus,
Piso, and Philippus were still on the road to Antony at
Mutina, called on small business about the Appian Way and the
Mint and the Lupercalia but used to keep the will to war alive
--- and the speech's three-part architecture: \S 1--7 the
proem and encomium of Pansa, \S 8 the tripartite
\textit{propositio} (\textit{pacem cum M. Antonio esse nolo}
because it is \textit{turpis}, \textit{periculosa}, and
\textit{esse non potest}), \S 9--15 the demonstration of
shamefulness (the five-fold anaphora of \textit{non hostem
iudicastis}), \S 16--20 of danger (the Lucius-Antonius
patronage catalogue, redeploying Philippic 6), \S 21--25 of
impossibility (the cascade of constituencies with whom no
peace can hold), \S 26--27 the peroration with the
conditions Antony would have to meet and the warning to
Pansa at the tiller). Status flipped
\texttt{pending}~$\rightarrow$~\texttt{drafted};
\texttt{backfill\_structural.py} regenerated the parallel
sidecar (27 segments,
\texttt{data/parallel/speeches/043bc-philippic-7.json}, 37KB);
\texttt{build\_manifest.py} and \texttt{validate.py} both run
clean (8 warnings: pre-existing orphan-Latin for
\texttt{054bc-ad-familiares-13-49} plus the chronological-gap
warning, now at 40 pending earlier-dated works, down from 41
post-session-41, the expected $-1$ decrement). \textbf{Latest
drafted by deep chronology is unchanged}: \textit{Ad Familiares}
10.24 (28 July 43 BC) remains the marker, since
\textit{Philippic 7} is dated mid-January 43 BC and so falls
inside the chronological-gap zone.

\textbf{Cowork session 42 --- 1 substantial speech completed
(PM-direct, 27 sections):}

\begin{itemize}
\item \textbf{PM-direct (\textit{Philippic 7}, mid-January 43
BC, the senatorial holding speech of the legation interval ---
delivered while the legates Sulpicius Rufus, Piso, and
Philippus were on the road to Antony at Mutina, before any
report had come back, prepares the Senate for the inevitable
refusal):} 27 sections, $\sim$4{,}050 English words. A clean
tripartite \textit{sententia}: peace with Antony is
\textit{turpis}, \textit{periculosa}, \textit{esse non
potest} (\S 8, the \textit{propositio}). The first part
(\S 9--15) is shamefulness: the great five-fold anaphora of
\textit{non hostem iudicastis} (\S 10--13) running through
the chain of Senate decrees by which Antony has effectively
been judged an enemy --- the honours to Octavian, the praise
of the veterans and of the legions who deserted Antony, the
elevation of Brutus and the Gallic province, the order that
the consuls go to war, the universal levy --- and the closing
\textit{moriamur} (\S 14) as the rhetorical apex: if we cannot
wrench these arms away, let us die. The middle (\S 16--20)
is danger: the Antonii in Rome, Lucius the murmillo
\textit{patronus} (now compressed into a four-fold
\textit{patronus} anaphora, the statue-by-statue unfolding of
Philippic 6 condensed), the Mylasa murmillo who cut down his
Thracian-armed friend and took a \textit{luculenta plaga} in
return, the warning that if we slip in counsel now, the
wicked will seem to be too many soon. The close (\S 21--25)
is impossibility: a cascade of constituencies with whom no
peace can hold --- Senate, equestrians, the populus Romanus
in the contio, the municipalities, the named exemplum of
Lucius Visidius the Roman equestrian, Caesar Octavianus and
Decimus Brutus, the province of Gaul itself, all
\textit{plena odiorum, plena discordiarum}. The peroration
(\S 26--27) sets out the conditions Antony would have to meet
(cease attacking Brutus, allow legates to approach him, lead
the army across the Rubicon, come within 200 miles of Rome
but no nearer, submit to Senate and Roman people) and closes
with the brilliant reversal: \textit{non illi senatus, sed
ille populo Romano bellum indixerit} --- not the Senate
declaring war on him, but he on the Roman people --- and the
warning to Pansa at the tiller of the ship of state.
\textbf{Sidecar:} parallel-sidecar generated via
\texttt{backfill\_structural.py} (27 segments,
\texttt{data/parallel/speeches/043bc-philippic-7.json}, 37KB).
No Greek phrases in this speech (consistent with the
Philippics generally). No date correction needed (Perseus
TEI numbering for Philippic 7 is regular, no anomaly; the
\texttt{date\_precision: month} in works.yaml correctly
reflects the manuscript-tradition month-precision dating,
since no sharper internal indication exists in the text).
\end{itemize}

\textbf{No metadata corrections committed during session 42.}
The meta entry was already correct, month-precision; Perseus
TEI numbering for Philippic 7 is regular (no anomaly
analogous to the Fam.\ 13.49 case).

\textbf{Translator-notes worth logging in
\texttt{data/translator-notes.jsonl}} (Cowork did not write
the shared jsonl directly to avoid concurrent-write
contention; deferred to a future enrichment pass):
\begin{itemize}
\item \textbf{\S 1 \textit{parvis de rebus sed fortasse
necessariis consulimur}.} Rendered ``We are consulted on
small matters --- though perhaps necessary ones.''
\textit{Parvis $\ldots$ necessariis} is the deliberate
opening contrast: the day's business (Appian Way, Mint,
Lupercalia) is small, the larger crisis is not. The
\textit{sed fortasse} concedes the smallness without
dismissing it; the English needs to keep both poles
(``small $\ldots$ necessary'') in their balanced order.
\textit{Consulimur} as the technical Senate idiom ``we are
consulted'' (the presiding magistrate puts the question)
rather than the looser ``we are asked.''
\item \textbf{\S 1 \textit{animus aberrat a sententia
suspensus curis maioribus}.} Rendered ``my mind wanders
from the question, suspended on graver cares.'' \textit{Animus
aberrat} as ``my mind wanders'' (the standard Latin
metaphor, intelligible in English); \textit{a sententia} as
``from the question,'' the \textit{sententia} being the
matter on which one is to speak one's opinion (not the
opinion itself in this construction). \textit{Suspensus} as
``suspended,'' the participle of the mind hung between two
matters, the larger pulling on the smaller.
\item \textbf{\S 5 \textit{faveas tu hosti?} and
\textit{te consularem aut senatorem, denique civem
putes?}} The cascade of indignant rhetorical questions
about the man who would publish Antony's letters with
delight. Rendered \textit{would you favour an enemy?} and
\textit{after this think yourself a consular, or a senator,
or finally a citizen?} The subjunctive interrogatives kept
as ``would you,'' the climactic descent \textit{consularem
$\ldots$ senatorem $\ldots$ civem} kept in three steps,
the final \textit{civem} the lowest rung, since to be no
longer even a citizen is the full reproach.
\item \textbf{\S 8 \textit{ego ille $\ldots$ pacis semper
laudator, semper auctor, pacem cum M. Antonio esse nolo}.}
The \textit{propositio}, the architectural centre of the
speech. Rendered ``I, that man, I will say again and again,
always the praiser of peace, always its author, do not want
peace with Marcus Antonius.'' \textit{Ego ille} as ``I, that
man'' (the demonstrative is the rhetorical self-pointing, ``I,
that very same Cicero you all know to be the author of
peace''); \textit{laudator $\ldots$ auctor} kept as a
parallel pair (``praiser $\ldots$ author''); the closing
\textit{pacem $\ldots$ nolo} kept short and flat, as in the
Latin.
\item \textbf{\S 9 \textit{quia turpis est, quia periculosa,
quia esse non potest}.} The three reasons announced.
Rendered ``Because it is shameful, because it is dangerous,
because it cannot exist.'' \textit{Turpis} as ``shameful''
(the moral term, not ``ugly''); \textit{periculosa} as
``dangerous'' (plain); \textit{esse non potest} as ``it
cannot exist'' rather than ``it cannot be,'' since Cicero
is making the structural claim that no possible relation
between Antony and the Roman state can answer to the name
of peace.
\item \textbf{\S 10--13 \textit{non hostem iudicastis} ---
the five-fold anaphora}. Rendered with the repeating
formula ``did you not then judge Antony an enemy?'' / ``was
Antony not then judged an enemy?'' / etc. The English has to
keep the structural drumbeat, since the argument depends on
the listener feeling the iteration: each Senate decree was
an implicit declaration of enmity. \textit{Iudicare hostem}
as ``to judge an enemy'' (the technical formula by which the
Senate declared a citizen a public enemy), not ``to consider
him hostile.''
\item \textbf{\S 14 \textit{si non possumus facere $\ldots$
moriamur}.} The rhetorical apex. Rendered ``But if we cannot
do it --- I will say what is worthy of a senator and of a
Roman man --- let us die.'' \textit{Moriamur} as ``let us
die,'' the bare subjunctive, no padding (``let us perish''
would soften, ``let us die'' lands). The interjected
self-justification \textit{dicam quod dignum est et
senatore et Romano homine} kept exactly in position
between dashes, so the structure (the qualification, then
the verb) reads as in the Latin.
\item \textbf{\S 16 \textit{patronus quinque et triginta
tribuum $\ldots$ patronus centuriarum equitum Romanorum
$\ldots$ patronus eorum qui tribuni militares fuerunt
$\ldots$ patronus Iani medii}.} The four-fold
\textit{patronus} anaphora compressing Philippic 6's
statue-by-statue catalogue. Rendered ``patron of the
thirty-five tribes $\ldots$ patron of the centuries of
Roman equestrians $\ldots$ patron of those who were
military tribunes $\ldots$ patron of the Middle Janus.''
The English keeps \textit{patronus} $\to$ ``patron'' four
times, the structural drumbeat retained. The
intervening clauses (the \textit{suffragium sustulit}
about the equestrian voting reform) kept in their
subordinate position, so the four \textit{patronus}
heads remain audible.
\item \textbf{\S 17 \textit{Myrmillo in Asia depugnavit.
cum ornasset Thraecidicis comitem et familiarem suum,
illum miserum fugientem iugulavit, luculentam tamen ipse
plagam accepit, ut declarat cicatrix}.} The Mylasa arena
anecdote about Lucius Antonius. Rendered ``A Myrmillo, he
fought to the death in Asia. When he had armed his comrade
and intimate friend in the Thracian's gear, that wretch
fleeing he stabbed in the throat --- though he himself
took a luculent gash, as the scar declares.''
\textit{Myrmillo} kept as a transliterated proper noun (the
class of Roman gladiator, distinct from the
\textit{Thraex} class he was matched against);
\textit{depugnavit} as ``fought to the death'' (the
\textit{de-} prefix the technical term for a contest to
the finish, not a sparring bout); \textit{Thraecidicis}
as ``the Thracian's gear'' (the equipment of the
\textit{Thraex}); \textit{iugulavit} as ``stabbed in the
throat'' (the verb is anatomically specific);
\textit{luculenta plaga} kept as ``luculent gash,'' the
\textit{luculentus} adjective preserved as the Latinate
cognate the reader of expanded vocabulary will catch (a
``handsome'' wound, in the gallows-humour sense).
\item \textbf{\S 26 \textit{non illi senatus, sed ille
populo Romano bellum indixerit}.} The peroration's
brilliant reversal. Rendered ``then it is not the Senate
that has declared war on him, but he who has declared war
on the Roman people.'' \textit{Illi $\ldots$ ille} kept as
the contrast between the dative (war declared on him) and
the nominative (war declared by him); \textit{bellum
indicere} as the standard formal idiom ``to declare war,''
rendered as one phrase. The reversal is the architectural
hinge of the close: it is not we who are the warmakers but
the man we have already named an enemy in fact if not in
word.
\end{itemize}

\textbf{Suggested next translation batch} (when session 42 is
landed via \texttt{bash scripts/cowork\_handoff.sh "session 42:
\ldots"} and session 43 opens):

\textbf{Session 43 opens \textit{Philippic 8} (3 February 43
BC, $\sim$33 sections).} The senatorial \textit{tumultus}
speech delivered the day after the legates returned with
Antony's counter-demands; the Senate at last decrees
\textit{tumultus} (which Cicero had argued for unsuccessfully
on the Kalends of January and which Philippics 5--7 had
prepared the ground for). $\sim$33 sections fits a single
session in single-substantial-speech mode, six or seven
5-section Edit chunks.

\textbf{After \textit{Philippic 8}} (sessions 44--49 or so):
\textit{9} (4 Feb 43, $\sim$17 sections, on Sulpicius's
death), \textit{10} (Feb 43, $\sim$26 sections), \textit{11}
(Mar 43, $\sim$40 sections), \textit{12} (Mar 43, $\sim$30
sections), \textit{13} (20 Mar 43, $\sim$50 sections),
\textit{14} (21 Apr 43, $\sim$38 sections). Each fits a
single session (with the longer two --- 11 and 13 --- nearer
the upper bound). Total Philippic phase is now 7 sessions
left (Phil 3, 4, 5, 6, 7 now completed; Phil 8--14 to go).
...
3--4 parallel batches of worker sub-agents, each translating
3--5 short letters end-to-end}) remains stale and continues to
auto-pivot. Sessions 32--42 all dropped to single-substantial-
speech mode per this section's instructions; the same
auto-pivot will continue through the Philippic and
philosophical phases (sessions 43--60 or so). Updating the
scheduled-task prompt to reflect Phase 3 of OPERATING\_PLAN.md
(``one substantial-work worker per session, 5-section chunks,
...
## Where to resume now (session 41 legacy snapshot below)

\textbf{Translation state (post-Cowork-session-41):} \textbf{911 /
958 works drafted (\~95.1\%)} --- session 41 completed
\textit{Philippic 6} (4 January 43 BC, 19 sections,
$\sim$2{,}850 English words) end-to-end in one session: Latin
fetched (12{,}673 chars), English translated in
single-substantial-speech mode in four Edit chunks
(\textsection 1--5, 6--10, 11--15, 16--19), four-paragraph
headnote written (composition history --- the contio of
4 January 43 BC delivered to the people in the Forum three
days after \textit{Philippic 5}, after the Senate, against
Cicero's motion, had voted to send a legation of Sulpicius
Rufus, Piso, and Philippus to Antony at Mutina rather than
declare \textit{tumultus} outright --- and the speech's three-
part shape: \S 1--9 the report and case against the legation,
\S 10--15 the catalogue of Lucius Antonius's statues set up
by the thirty-five tribes, the equestrians, the military
tribunes, and the moneylenders of the Middle Janus, \S 16--19
the close on \textit{libertas}). Status flipped
\texttt{pending}~$\rightarrow$~\texttt{drafted};
\texttt{backfill\_structural.py} regenerated the parallel
sidecar (19 segments,
\texttt{data/parallel/speeches/043bc-philippic-6.json}, 30KB);
\texttt{build\_manifest.py} and \texttt{validate.py} both run
clean (8 warnings: pre-existing orphan-Latin for
\texttt{054bc-ad-familiares-13-49} plus the chronological-gap
warning, now at 41 pending earlier-dated works, down from 42
post-session-40, the expected $-1$ decrement). \textbf{Latest
drafted by deep chronology is unchanged}: \textit{Ad Familiares}
10.24 (28 July 43 BC) remains the marker, since
\textit{Philippic 6} is dated 4 January 43 BC and so falls
inside the chronological-gap zone.

\textbf{Cowork session 41 --- 1 substantial speech completed
(PM-direct, 19 sections):}

\begin{itemize}
\item \textbf{PM-direct (\textit{Philippic 6}, 4 January 43 BC,
the popular damage-control speech delivered to the people in
the Forum three days after the Kalends debate, reporting on
the compromise legation):} 19 sections, $\sim$2{,}850 English
words. A short \textit{contio} in three unequal parts. The
first half (\S 1--9) is Cicero making the best of a Senate
vote he opposed: the legation is at bottom a denunciation of
war, like the one once sent to Hannibal not to attack
Saguntum (\S 4, 6); Antony will not obey, because he has
never obeyed his own judgment let alone the Senate's
(\S 4--5); Decimus Brutus is the test, a Brutus born for
liberty (\S 8--9). The middle (\S 10--15) is the famous
set-piece on Lucius Antonius the murmillo brother, the
\textit{Africanus} of the Antonine inner circle, ridiculed
through his four (or five) public statues: the gilded
equestrian \textit{Of the thirty-five tribes to their patron}
(\S 12), the Forum statue beside that of Quintus Tremulus who
conquered the Hernici (\S 13), the equestrian-order statue
(\S 13), the military-tribunes' statue erected by men twice in
Caesar's army (\S 14), and the \textit{palmaris}
moneylenders' statue from the Middle Janus inscribed \textit{To
Lucius Antonius, from the Middle Janus, their patron} (\S 15)
--- with the Trebellius \textit{O fides!} digression
(\S 11) as bridge. The close (\S 16--19) is the great
peroration on liberty: \textit{populum Romanum servire fas non
est, quem di immortales omnibus gentibus imperare voluerunt
$\ldots$ aliae nationes servitutem pati possunt, populi
Romani est propria libertas} (\S 19). \textbf{Sidecar:}
parallel-sidecar generated via
\texttt{backfill\_structural.py} (19 segments,
\texttt{data/parallel/speeches/043bc-philippic-6.json}, 30KB).
No Greek phrases in this speech (consistent with the
Philippics generally; the closing \textit{libertas} sentence
is purely Latin). No date correction needed
(\texttt{-0043-01-04} day-precision was already correct).
\end{itemize}

\textbf{No metadata corrections committed during session 41.}
The meta entry was already correct, day-precision; Perseus
TEI numbering for Philippic 6 is regular (no anomaly
analogous to the Fam.\ 13.49 case).
...
contention; deferred to a future enrichment pass):
\begin{itemize}
\item \textbf{\S 1 \textit{minus quidem illa severe quam
decuit, non tamen omnino dissolute}.} Rendered ``less
severely, indeed, than was fitting, but still not altogether
weakly.'' \textit{Dissolute} is the contrast term to
\textit{severe} and means here ``slackly, weakly'' (the
\textit{dissolutus} as the man without resolve, not the
\textit{dissolutus} as the wastrel). The judgment is the
controlled, slightly bitter one Cicero uses repeatedly through
the speech --- restated at \S 7 with \textit{non omnino
dissolutum est quod decrevit senatus: habet atrocitatis
aliquid legatio} --- the bicolon of \textit{severe} versus
\textit{dissolute} the architecture of the whole speech. The
English needs to keep both poles intact.
\item \textbf{\S 2 \textit{ut meminissem quam personam
impositam a vobis sustinerem}.} The theatrical metaphor of
\textit{persona} (the actor's mask). Rendered ``so that I
might remember what part you had imposed upon me to
sustain.'' \textit{Persona} as ``part'' (the actor's
function, not the modern ``person''); \textit{impositam $\ldots$
sustinerem} kept as a unit, the \textit{sustinere personam}
being the standard idiom for ``to sustain a role.''
\item \textbf{\S 3 \textit{quam ob rem $\ldots$ quae vobis
potest cum hoc gladiatore condicionis, aequitatis, legationis
esse communitas?}} The closing tricolon on the negotiation
with Antony. Rendered ``What common ground of terms, of fair
dealing, of negotiation can you have with this gladiator?''
\textit{Condicio} as ``terms'' (the technical term for terms
of agreement, not ``condition''); \textit{aequitas} as ``fair
dealing'' (the procedural-fairness sense, not modern
``equality''); \textit{legatio} as ``negotiation'' (the
function of an embassy, not its bare name, since the legation
\textit{is} what's at issue). \textit{Gladiator} kept literal
--- the abuse-word recurring through Philippics 2 and 3.
\item \textbf{\S 4 \textit{semper eo tractus est quo libido
rapuit, quo levitas, quo furor, quo vinolentia}.} The four-
part anaphora of \textit{quo}. Rendered ``He has always been
dragged where lust dragged him, where fickleness, where
frenzy, where drunkenness.'' The English keeps \textit{quo}
$\to$ ``where'' four times; \textit{libido / levitas / furor /
vinolentia} as ``lust / fickleness / frenzy / drunkenness'' ---
each a single English noun, no doubling, since the rhythm of
the Latin is the architecture and the English nouns need to
land singly to preserve it. \textit{Vinolentia} specifically
as ``drunkenness'' (not ``love of wine'' or ``intoxication'')
to give it the same monosyllabic weight as \textit{furor}.
\item \textbf{\S 7 \textit{nec vero de illo sicut de homine
aliquo debemus, sed ut de importunissima belua cogitare}.}
Rendered ``Nor indeed ought we to think of him as we do of
any human being, but as of some most savage beast.''
\textit{Importuna belua} is the bestiality-of-Antony figure
that runs through the speech and culminates in the Hannibal-
beast comparison of Philippic 4. \textit{Importunissimus} as
``most savage'' (rather than ``inopportune'' or
``troublesome,'' which miss the predatory force); \textit{belua}
as ``beast'' kept simple. The whole structure --- ``not as of
a man, but as of a beast'' --- preserved as a contrast pair.
\item \textbf{\S 10 \textit{nam hic inter illos Africanus
est}.} Of Lucius Antonius among Antony's lieutenants.
Rendered ``For this man, among those of Antony's circle, is
an Africanus.'' \textit{Africanus} kept as the bare cognomen
--- the sarcasm depends on the reader hearing the name of
the greatest Roman general and seeing it applied to a
gladiator who killed his friend at Mylasa. To translate
\textit{Africanus} as ``a chief'' or ``a paragon'' would
flatten the irony; the Latin name carries it in English too.
\item \textbf{\S 11 \textit{O fide! --- hoc enim opinor
Trebellium sumpsisse cognomen}.} The Trebellius pun.
Rendered ``O Faith! --- for this, I think, is the cognomen
Trebellius assumed.'' The joke is that Trebellius's cognomen
sounds nothing like \textit{fides}; Cicero pretends it does,
sarcastically, since Trebellius has acted in every way
opposite to faith. The English needs to preserve the joke's
shape (\textit{O fides!} as exclamation, the supposed
etymology, the punchline of the defrauded creditors), which
it does if \textit{fides} is translated as ``faith'' rather
than ``credit'' or ``loyalty'' --- ``faith'' lets the
cognomen-pun land, ``credit'' wouldn't.
\item \textbf{\S 12 inscription \textit{Qvinqve et triginta
tribvs patrono} and \S 15 inscription \textit{L.\ Antonio a
Iano medio patrono}.} The two statue inscriptions. Rendered
in \textit{italic} (``To their patron from the thirty-five
tribes,'' ``To Lucius Antonius, from the Middle Janus, their
patron'') to mark them as quoted inscriptions rather than
Cicero's own voice. The \textit{equo publico} equestrian
inscription at \S 13 (\textit{patrono}) likewise marked
\textit{italic}.
\item \textbf{\S 15 \textit{quis umquam in illo Iano inventus
est qui L.\ Antonio mille nummum ferret expensum?}} The
moneylender joke. Rendered ``Who was ever found in that
Janus to enter on his books a thousand sesterces lent to
Lucius Antonius?'' \textit{Expensum ferre} is the technical
accounting idiom of the Roman moneylender --- to record on
the credit side of one's ledger an amount paid out as a loan
--- so the English needs to land in the bookkeeping register
(``enter on his books $\ldots$ lent'') rather than collapse to
``lend.'' The point is that no one in the Middle Janus would
have lent Lucius Antonius a sesterce; the statue inscribing
him as patron of the place is therefore self-evidently absurd.
\item \textbf{\S 17 \textit{si pacem adferent, cupidum me; si
bellum, providum iudicatote}.} The pivot couplet. Rendered
``if they bring peace, judge me eager for it; if war, judge
me foreseeing.'' \textit{Cupidum / providum} as ``eager /
foreseeing'' --- the contrast is between desire and
foreknowledge, both Latin words ending in \textit{-um}; the
English ``eager / foreseeing'' is the closest balance
available, the parallel weakened only slightly by the change
of suffix. \textit{Iudicatote} (future imperative) preserved
as ``judge'' rather than collapsed to a flat indicative.
\item \textbf{\S 19 \textit{populum Romanum servire fas non
est, quem di immortales omnibus gentibus imperare voluerunt}.}
The peroration. Rendered ``It is not divinely lawful for the
Roman people to be slaves --- whom the immortal gods willed
to rule over all nations.'' \textit{Fas non est} as ``it is
not divinely lawful'' (rather than ``it is not right'' or
``it is not lawful'') to preserve the religious-civic charge:
\textit{fas} is the divine-law term, and Cicero is invoking
the gods explicitly in the next clause. The closing
\textit{aliae nationes servitutem pati possunt, populi Romani
est propria libertas} kept as a balanced bicolon (``Other
nations can bear slavery; liberty belongs to the Roman people
by right''), with \textit{propria} as ``by right'' rather than
``their own'' --- the property-language sense of
\textit{proprius}, what belongs to one's nature and not to
another.
\end{itemize}

\textbf{Suggested next translation batch} (when session 41 is
landed via \texttt{bash scripts/cowork\_handoff.sh "session 41:
\ldots"} and session 42 opens):

\textbf{Session 42 opens \textit{Philippic 7} (mid-January 43
BC, $\sim$27 sections).} The Senate speech delivered while
the legates are away at Mutina, the great anti-peace
\textit{sententia} on what should be done if Antony refuses
the terms --- the holding speech of the legation interval.
$\sim$27 sections fits comfortably in a single session in
single-substantial-speech mode, five or six 5-section Edit
chunks.

\textbf{After \textit{Philippic 7}} (sessions 43--49 or so):
\textit{8} (3 Feb 43, $\sim$33 sections, the \textit{tumultus}
speech after the legates' return), \textit{9} (4 Feb 43,
$\sim$17 sections, on Sulpicius's death), \textit{10}
(Feb 43, $\sim$26 sections), \textit{11} (Mar 43, $\sim$40
sections), \textit{12} (Mar 43, $\sim$30 sections),
\textit{13} (20 Mar 43, $\sim$50 sections), \textit{14}
(21 Apr 43, $\sim$38 sections). Each fits a single session
(with the longer two --- 11 and 13 --- nearer the upper
bound). Total Philippic phase is likely 8 sessions left
(Phil 3, 4, 5, 6 now completed, Phil 7--14 to go).
...
3--4 parallel batches of worker sub-agents, each translating
3--5 short letters end-to-end}) remains stale and continues to
auto-pivot. Sessions 32--41 all dropped to single-substantial-
speech mode per this section's instructions; the same
auto-pivot will continue through the Philippic and
philosophical phases (sessions 42--60 or so). Updating the
scheduled-task prompt to reflect Phase 3 of OPERATING\_PLAN.md
(``one substantial-work worker per session, 5-section chunks,
WIP-emit discipline, multi-session splits allowed for very
long works'') is recommended but not blocking --- the PM in
...
## Where to resume now (session 40 legacy snapshot below)

\textbf{Translation state (post-Cowork-session-40):} \textbf{910 /
958 works drafted (\~95.0\%)} --- session 40 completed
\textit{Philippic 5} (1 January 43 BC, 53 sections,
$\sim$5{,}900 English words) end-to-end in one session: Latin
fetched (37{,}465 chars), English translated in
single-substantial-speech mode in eleven Edit chunks of five
sections each (with a final three-section closing chunk
\textsection 51--53), four-paragraph headnote written
(composition history --- the inaugural Senate meeting of Pansa
and Hirtius in the temple of Concord on the Kalends, the
Calenus proposal to send a legation to Antony rather than
proceed to war, Cicero's rebuttal --- and the speech's
two-half structure: \S 1--34 the case against the embassy and
for war, with \S 7--16 the long catalogue of Antony's
consular crimes culminating in the third-panel-of-judges
\textit{set piece}, \S 17--22 the armed gangs and the
Brundisium centurion-killings, \S 23--25 the Hannibal
comparison, \S 26--34 the practical case and Cicero's motion
of \textit{tumultus}/\textit{iustitium}/\textit{saga}/\textit{dilectus};
then \S 35--53 the second half, the drafted senatus consulta
honouring Decimus Brutus, Lepidus, Octavian (\S 42--51, the
longest single block of the speech, including \textit{
promitto, recipio, spondeo} as Cicero's personal guarantee of
Caesar's future conduct), Egnatuleius, and the veterans of
the Martian and Fourth Legions). Status flipped \texttt{pending}
~$\rightarrow$~\texttt{drafted}; \texttt{backfill\_structural.py}
regenerated the parallel sidecar (53 segments,
\texttt{data/parallel/speeches/043bc-philippic-5.json}, 87KB);
\texttt{build\_manifest.py} and \texttt{validate.py} both run
clean (8 warnings: pre-existing orphan-Latin for
\texttt{054bc-ad-familiares-13-49} plus the chronological-gap
warning, now at 42 pending earlier-dated works, down from 43
post-session-39, the expected $-1$ decrement). \textbf{Latest
drafted by deep chronology is unchanged}: \textit{Ad Familiares}
10.24 (28 July 43 BC) remains the marker, since
\textit{Philippic 5} is dated 1 January 43 BC and so falls
inside the chronological-gap zone.

\textbf{Cowork session 40 --- 1 substantial speech completed
(PM-direct, 53 sections):}

\begin{itemize}
\item \textbf{PM-direct (\textit{Philippic 5}, 1 January 43
BC, the inaugural-Senate-meeting speech of the consular year
of Pansa and Hirtius, the great rebuttal of the Calenus
embassy proposal):} 53 sections, $\sim$5{,}900 English words.
The longest Philippic so far in the cycle, and the most
legislative in shape: not the indignation of Philippic 4 nor
the formal motion of Philippic 3, but a sustained debate-
speech that begins with the case against sending a legation
to Antony, runs through Antony's whole record of public
violence as consul, and closes with a series of drafted
senatus consulta honouring those already in arms. The two
halves are unequal --- \textsection 1--34 against the
embassy and for war (with the great extended caricature of
Antony's third panel of judges at \S 12--16, Cydas of Crete
and Lysiades the Athenian named, and the cascading practical
argument at \S 26--34 that a legation will quench the fire of
the Roman people and break the spirit of the municipalities);
then \S 35--53 the legislative honours --- Decimus Brutus
(praise for holding Gaul), Lepidus (a gilded equestrian
statue for restoring Sextus Pompey), Gaius Caesar at length
(\S 42--51, propraetorian imperium and a seat in the Senate
with consular rank, with Alexander and Africanus and
Flamininus brought in to defend the unprecedented honours),
Egnatuleius (three years before the legal age), and the
veterans of the Martian and Fourth Legions (\S 53, the long
final s.c.). \textbf{Sidecar:} parallel-sidecar generated via
\texttt{backfill\_structural.py} (53 segments,
\texttt{data/parallel/speeches/043bc-philippic-5.json}, 87KB).
No Greek phrases in this speech (consistent with the
Philippics generally). No date correction needed
(\texttt{-0043-01-01} day-precision was already correct).
\end{itemize}

\textbf{No metadata corrections committed during session 40.}
The meta entry was already correct, day-precision.
...
deferred to a future enrichment pass):
\begin{itemize}
\item \textbf{\S 3 \textit{non iam levitatis est, sed, ut quod
sentio dicam, dementiae}.} Rendered ``is no longer mere
fickleness, but, to speak my mind, derangement.''
\textit{Levitas} is the lighter charge (mere inconsistency
between the s.c.s of 20 December and a legation now);
\textit{dementia} is the heavier --- not ``madness'' (which
in English drains the legal-political charge) but the
faculty-of-mind term ``derangement.'' Cicero is gradating
the indictment, and the English nouns need to gradate too.
\item \textbf{\S 5 \textit{hoc qui non videt, excors, qui cum
videt decernit, impius est}.} The closing dichotomy on giving
Antony Plancus's Gaul. Rendered ``He who does not see this is
brainless; he who, seeing, decrees it, is impious.'' \textit{
Excors} as ``brainless'' (rather than ``heartless,'' which
mistakes the etymology --- \textit{cor} here as the seat of
intelligence, not affection); \textit{impius} kept as
``impious,'' which preserves the religious-civic weight that
the simple ``wicked'' or ``unprincipled'' would lose.
\item \textbf{\S 12--16 the third panel of judges (\textit{
tertia decuria}).} The most savagely funny set-piece in the
speech, naming Cydas of Crete and Lysiades the Athenian (the
son of Phaedrus the Epicurean), and ridiculing the Greeks,
gamblers, exiles, dancers, and lyre-players whom Antony has
filed with the treasury as iudices. Rendered with the cast of
characters preserved by name (\textit{Cydam amo Cretensem,
portentum insulae} as ``I am fond of Cydas of Crete, the
prodigy of his island''; \textit{ne Curium quidem esse
crudelem qui periculum fortunae cotidie facit} as ``Curius is
not cruel either, who takes his chance with Fortune every
day,'' preserving the gambling joke). \textit{Pallium /
toga} kept as ``pallium and toga'' (the Greek cloak and the
Roman, with no English equivalent that lands the cultural
contrast).
\item \textbf{\S 17 \textit{Cinnam memini; vidi Sullam; modo
Caesarem}.} The tricolon of past strongmen. Rendered ``I
remember Cinna; I saw Sulla; only the other day, Caesar.''
The three verbs gradate from \textit{memini} (childhood
memory --- Cicero was eight when Cinna was killed in 84) to
\textit{vidi} (his late teens and twenties, when Sulla was
dictator) to the bare \textit{modo} for the recent Caesar:
preserved as ``remember,'' ``saw,'' and an unmediated bare
noun, which lands the increasing immediacy without forcing
the verb sequence into English.
\item \textbf{\S 25 \textit{ergo Hannibal hostis, civis
Antonius?}} The pivot of the first half. Rendered ``Is
Hannibal then an enemy, and Antony a citizen?'' The
asyndetic Latin (no copula, no ``then'' in the question)
opened up with ``Is~$\ldots$~then~$\ldots$~and~$\ldots$'' for
English clarity, but the rhetorical pair \textit{hostis /
civis} kept verbatim and the question form preserved. The
whole inferential structure of the section --- Hannibal was
restrained by self-interest, Antony's faction is not ---
preserved without softening.
\item \textbf{\S 31 \textit{tumultum decerni, iustitium
edici, saga sumi dico oportere, dilectum haberi sublatis
vacationibus}.} The four-part motion. Rendered ``I say that
a state of tumult must be decreed, the courts suspended,
the military cloaks taken up, a levy held, exemptions
revoked.'' \textit{Tumultus} kept literal (``state of
tumult''), since it is the technical term for the formal
declaration that an emergency in Italy or Gaul exists;
\textit{iustitium} as ``the courts suspended'' (rather than
``a iustitium declared,'' which would over-Latinise);
\textit{saga sumere} as ``take up the military cloaks''
(the standard idiom for the change from the civilian toga
to the military sagum); \textit{dilectus} kept as ``levy.''
\item \textbf{\S 38 \textit{cum Antonio diadema Caesari
imponente se avertit gemituque et maestitia declaravit}.}
Lepidus's reaction at the Lupercalia. Rendered ``when, while
Antony was placing the diadem on Caesar, he turned aside and
by his groans and his sadness made clear $\ldots$'' --- the
present participle \textit{imponente} kept as a temporal
``while~$\ldots$ was placing'' (rather than collapsing to
``Antony's offering of the diadem,'' which would lose the
moment of action). The whole passage is a careful
exoneration of Lepidus, set up to license the gilded statue
in \S 41.
\item \textbf{\S 51 \textit{promitto, recipio, spondeo,
patres conscripti}.} Cicero's personal guarantee of the
young Caesar's future conduct. Rendered ``I promise, I
undertake, I pledge, senators.'' Three Latin verbs $\to$
three English verbs, each weightier than the last --- the
\textit{tricolon crescendo} from a private undertaking
(\textit{promitto}) through assuming-the-responsibility
(\textit{recipio}) to the formal solemn pledge with
religious weight (\textit{spondeo}, the verb of sponsion).
The reader who knows what the next eighteen months will
do to this guarantee should be allowed to feel its
solemnity intact, not have it softened.
\item \textbf{\S 53 the closing senatus consultum on the
Martian and Fourth Legions.} Reproduced as a single
unbroken legislative period in English (no break-out of
sub-clauses) so that the cumulative weight of the package
of exemptions, lands, and bonuses --- and the procedural
device that the consuls Pansa and Hirtius are to draw up
the details \textit{alter ambove, si eis videretur} ---
lands as a single drafted s.c. The closing \textit{dixi ad
ea omnia, consules, de quibus rettulistis}: rendered ``I
have spoken, consuls, on all those matters concerning
which you have referred to us,'' the senatorial-protocol
formula that closes the contribution.
\end{itemize}

\textbf{Suggested next translation batch} (when session 40 is
landed via \texttt{bash scripts/cowork\_handoff.sh "session 40:
\ldots"} and session 41 opens):

\textbf{Session 41 opens \textit{Philippic 6} (4 January 43 BC,
$\sim$19 sections).} A short \textit{contio} speech delivered
to the people in the Forum on 4 January 43, three days after
Philippic 5, after the Senate had ended by sending the
legation (Sulpicius, Piso, Philippus) to Antony despite
Cicero's opposition --- the compromise outcome that Philippic
5 had argued against. The speech is Cicero's report to the
people of what was decided, of what was decided differently
from what he had urged, and of why the people should
nevertheless not be discouraged: war is still coming, the
legation is only a brief delay, the Senate's resolve is firm.
Short --- $\sim$19 sections fits comfortably in a single
session in single-substantial-speech mode, three to four
5-section Edit chunks.

\textbf{After \textit{Philippic 6}} (sessions 42--49 or so):
\textit{7} (Jan 43, $\sim$27 sections), \textit{8} (3 Feb 43,
$\sim$33 sections), \textit{9} (4 Feb 43, $\sim$17 sections),
\textit{10} (Feb 43, $\sim$26 sections), \textit{11} (Mar 43,
$\sim$40 sections), \textit{12} (Mar 43, $\sim$30 sections),
\textit{13} (20 Mar 43, $\sim$50 sections), \textit{14} (21 Apr
43, $\sim$38 sections). Each fits a single session (with the
longer two --- 11 and 13 --- nearer the upper bound). Total
Philippic phase is likely 9 sessions left (Phil 3, 4, and 5
now completed, Phil 6--14 to go).
...
3--4 parallel batches of worker sub-agents, each translating
3--5 short letters end-to-end}) remains stale and continues to
auto-pivot. Sessions 32--40 all dropped to single-substantial-
speech mode per this section's instructions; the same
auto-pivot will continue through the Philippic and
philosophical phases (sessions 41--60 or so). Updating the
scheduled-task prompt to reflect Phase 3 of OPERATING\_PLAN.md
(``one substantial-work worker per session, 5-section chunks,
WIP-emit discipline, multi-session splits allowed for very
long works'') is recommended but not blocking --- the PM in
...
## Where to resume now (session 39 legacy snapshot below)

\textbf{Translation state (post-Cowork-session-39):} \textbf{909 /
958 works drafted (\~94.9\%)} --- session 39 completed
\textit{Philippic 4} (20 December 44 BC, 16 sections,
$\sim$2{,}180 English words) end-to-end in one session: Latin
fetched (10{,}096 chars), English translated in
single-substantial-speech mode in three Edit chunks
(\textsection 1--5, 6--10, 11--16), four-paragraph headnote
written (composition history --- delivered hours after the
Third Philippic on 20 December 44 BC, the Senate-then-Forum
sequence with Cicero descending from the Curia to the Rostra ---
and the speech's structure: \S 1--6 the inferential
\textit{hostis} argument, \S 7--9 the converging judgments
of Decimus Brutus / Gaul / Italy, \S 10 the gods, \S 11--16
the hortatory close on the beast-in-a-pit and the contrast
with Catiline). Status flipped \texttt{pending}~$\rightarrow$~
\texttt{drafted}; \texttt{backfill\_structural.py} regenerated
the parallel sidecar (16 segments,
\texttt{data/parallel/speeches/044bc-philippic-4.json}, 23KB);
\texttt{build\_manifest.py} and \texttt{validate.py} both run
clean (8 warnings: pre-existing orphan-Latin for
\texttt{054bc-ad-familiares-13-49} plus the chronological-gap
warning, now at 43 pending earlier-dated works, down from 44
post-session-38, the expected $-1$ decrement). \textbf{Latest
drafted by deep chronology is unchanged}: \textit{Ad Familiares}
10.24 (28 July 43 BC) remains the marker, since
\textit{Philippic 4} is dated 20 December 44 BC and so falls
inside the chronological-gap zone.

\textbf{Cowork session 39 --- 1 substantial speech completed
(1 worker / PM direct, 16 sections):}

\begin{itemize}
\item \textbf{PM-direct (\textit{Philippic 4}, 20 December 44
BC, the popular counterpart to the Third Philippic delivered
to the people in the Forum on the very same day, only hours
after the senatorial address):} 16 sections, $\sim$2{,}180
English words. The Senate meeting in the morning carried
Cicero's motion that Octavian, the Martian and Fourth Legions,
Lucius Egnatuleius, Decimus Brutus, and the Gallic towns be
commended for their actions against Antony. Cicero then went
down into the Forum and announced the result to the crowd that
had gathered there. The whole speech is built around a single
proposition stated in \S 1: \textit{est hostis a senatu nondum
verbo appellatus, sed re iam iudicatus Antonius} (``Antony has
not yet been called an enemy by the Senate in word, but in
fact he has already been so judged''). \S 1--6 the inferential
chain: armies have rightly been raised against Antony; the
Senate has just praised those who raised them; therefore by
inescapable inference the Senate has judged him an enemy ---
and the Martian and Fourth Legions, in deserting him for
Octavian and taking their stand at Alba, judged him an enemy
before the Senate did. \S 7--9 the converging judgments of
Decimus Brutus, the province of Gaul, all Italy, and the
people themselves. \S 10 the gods --- prodigies and a universal
consent which cannot have been without divine prompting. \S 11
--16 the hortatory close: Antony is no longer a wicked man but
a beast in a pit (\textit{immani taetraque belua quae, quoniam
in foveam incidit, obruatur}, \S 12); the contest is with an
assassin, a brigand, a Spartacus; the comparison with Catiline
replaces the Tarquin comparison of the Third Philippic and is
sharpened into a boast (Catiline at least raised an army from
nothing, whereas Antony has lost the army he was given);
\textit{me auctore et principe} closes on Cicero's own role
after the long withdrawal. \textbf{Sidecar:} parallel-sidecar
generated via \texttt{backfill\_structural.py} (16 segments,
\texttt{data/parallel/speeches/044bc-philippic-4.json}, 23KB).
No Greek phrases in this speech (consistent with the Philippics
generally). No date correction needed (\texttt{-0044-12-20}
day-precision was already correct).
\end{itemize}

\textbf{No metadata corrections committed during session 39.}
The meta entry was already correct, day-precision.
...
deferred to a future enrichment pass):
\begin{itemize}
\item \textbf{\textit{Quirites}, address throughout.} Rendered
``citizens'' consistently, parallel to the rendering of
\textit{patres conscripti} as ``senators.'' \textit{Quirites}
is the formal civic address to the people in their civilian
capacity, and the modern English ``citizens'' lands the
register without the antiquarian flavour of ``Romans'' or
``men of Rome.'' The choice carries through every speech to
the people; flagged here as a recurring decision rather than
a per-passage crux.
\item \textbf{\S 1 \textit{est hostis a senatu nondum verbo
appellatus, sed re iam iudicatus Antonius}.} The thesis
sentence of the whole speech. Rendered ``Antony has not yet
been called an enemy by the Senate in word, but in fact he has
already been so judged.'' The \textit{verbo / re} antithesis
(in word vs.\ in fact) preserved as ``in word / in fact''; the
\textit{nondum~$\ldots$~iam} pair (not yet~$\ldots$~already)
preserved. \textit{Hostis} rendered as the technical ``enemy''
(public enemy) rather than the bare ``foe'' --- though here,
unlike in Philippic 3 \S 14, ``public enemy'' would over-
specify and pre-empt the inferential argument; the simple
``enemy'' lets the inference build to its conclusion.
\item \textbf{\S 3 \textit{nomen clarissimi adulescentis vel
pueri potius; sunt enim facta eius immortalitatis, nomen
aetatis}.} Rendered ``the name of that most illustrious young
man --- or boy, rather; for his deeds are those of
immortality, his name those of his age.'' The
\textit{adulescentis / pueri potius} self-correction echoes
the famous \textit{adulescens, paene potius puer} of
Philippic 3 \S 3 and is preserved; the \textit{facta
immortalitatis / nomen aetatis} chiasmus (deeds belong to
immortality, name to youth) kept by parallel English nouns.
\item \textbf{\S 5 \textit{legio Martia, quae mihi videtur
divinitus ab eo deo traxisse nomen a quo populum Romanum
generatum accepimus}.} The etymological play on the Martian
Legion's name (Mars, ancestor of Romulus and so of the Roman
people). Rendered ``the Martian Legion --- which seems to me
to have drawn its name, by divine appointment, from that god
from whom we are taught the Roman people was begotten.''
\textit{Divinitus} as ``by divine appointment'' (rather than
``divinely'' or ``by the gods,'' which would either weaken or
over-specify); the legion's name is not glossed as ``from
Mars'' because the rhetorical figure is the periphrastic
identification, and to gloss it is to deflate it.
\item \textbf{\S 9 \textit{negat hoc D.~Brutus imperator,
consul designatus, natus rei publicae civis; negat Gallia,
negat cuncta Italia, negat senatus, negatis vos}.} The
anaphoric tricolon-plus-two on \textit{negat}. Rendered
``Decimus Brutus, commander, consul-elect, a citizen born for
the commonwealth, denies this; Gaul denies it; all Italy
denies it; the Senate denies it; you deny it.'' Five
\textit{negat/negatis} → five English ``denies'' (one ``deny''
on the second-person plural to match the form-shift). The
cascade of subjects (from Decimus Brutus through Gaul and
Italy and the Senate to the addressed crowd) is the load-
bearing rhetorical figure and is preserved verbatim.
\item \textbf{\S 12 \textit{immani taetraque belua quae,
quoniam in foveam incidit, obruatur}.} The beast-in-a-pit
image. Rendered ``a vast and foul beast which, since it has
fallen into the pit, must be buried under it.'' The
\textit{fovea} (the pit dug by hunters to trap large game)
kept as ``the pit'' (definite article: the trap that the
hunting metaphor implies); \textit{obruatur} as ``must be
buried under it'' rather than ``crushed'' or ``overwhelmed,''
to keep the physical specificity of the hunters' fill-in-
the-pit-with-stones gesture, which is the load-bearing image
of the second clause (the next sentence \textit{si enim illim
emerserit} --- ``if he climbs out of there'' --- depends on
the literal pit).
\item \textbf{\S 13 \textit{Alia omnia falsa, incerta sunt,
caduca, mobilia: virtus est una altissimis defixa radicibus,
quae numquam vi ulla labefactari potest, numquam demoveri
loco}.} The contrast of false-perishable things with valour-
rooted-deep. Rendered ``All other things are false, uncertain,
perishable, shifting: valour alone is set with the deepest
roots; she can never by any force be shaken, never moved from
her place.'' The four-adjective catalogue (\textit{falsa,
incerta, caduca, mobilia}) preserved as four adjectives; the
\textit{numquam~$\ldots$~numquam} anaphora preserved.
\item \textbf{\S 14 \textit{senatum, id est orbis terrae
consilium, delere gestit, ipse consilium publicum nullum
habet}.} The Senate-as-council-of-the-world image. Rendered
``he longs to destroy the Senate (that is, the council of the
whole world), while himself having no public council.''
\textit{Orbis terrae consilium} kept literally; the parenthesis
``(that is, $\ldots$)'' for the \textit{id est} apposition
makes the figure self-glossing for the reader. The
\textit{consilium}-pun (the Senate as \textit{consilium}, vs.\
Antony's lack of any \textit{consilium publicum}) preserved.
\item \textbf{\S 15 \textit{omne certamen cum percussore, cum
latrone, cum Spartaco}.} The triple of insulting names.
Rendered ``with an assassin, with a brigand, with a Spartacus.''
\textit{Percussor} kept as ``assassin'' (not ``murderer,''
which loses the political-killing flavour); \textit{latro} as
``brigand'' (a recurring Ciceronian term for Antony and his
followers, distinct from the more general ``robber'');
\textit{Spartacus} kept as a proper name with the indefinite
article (``a Spartacus''), which renders the noun-of-type idiom
better than the bare proper name and was a Ciceronian
commonplace.
\item \textbf{\S 16 \textit{longo intervallo me auctore et
principe ad spem libertatis exarsimus}.} The closing cadence,
echoing the Third Philippic \S 28 \textit{hodierno die primum,
patres conscripti, longo intervallo in possessionem libertatis
pedem ponimus}. Rendered ``for the first time after a long
interval, with me as proposer and leader, we have caught fire
with the hope of liberty.'' \textit{Exarsimus} as ``caught
fire'' (rather than ``blazed up,'' which over-translates the
inceptive); \textit{me auctore et principe} kept literal,
since both \textit{auctor} (the proposer of the motion) and
\textit{princeps} (the leading-from-the-front leader) are
load-bearing technical terms that the speech has earned.
\end{itemize}

\textbf{Suggested next translation batch} (when session 39 is
landed via \texttt{bash scripts/cowork\_handoff.sh "session 39:
\ldots"} and session 40 opens):

\textbf{Session 40 opens \textit{Philippic 5} (1 January 43 BC,
$\sim$53 sections).} The first speech of the consular year of
Pansa and Hirtius, delivered at the inaugural Senate meeting.
This is the long debate-speech opposing the Calenus proposal
of an embassy to Antony; the centrepiece is the case for war
on Antony rather than negotiation, and the speech also moves
the formal extraordinary honours for Octavian (the imperium
and the legions' confirmation) and for Decimus Brutus.
Substantial --- $\sim$53 sections is towards the upper bound
for one session; the single-substantial-speech pattern
continues, with the speech translated in $\sim$5-section
chunks (so $\sim$11 chunks across the session), WIP commits at
chunk boundaries, headnote at close. If the session budget
runs short, a clean stop at a chunk boundary (e.g.\ \S 25 or
\S 30) and a second session to finish is the documented
escape hatch.

\textbf{After \textit{Philippic 5}} (sessions 41--49 or so):
\textit{6} (4 Jan 43, $\sim$19 sections), \textit{7} (Jan 43,
$\sim$27 sections), \textit{8} (3 Feb 43, $\sim$33 sections),
...
Apr 43, $\sim$38 sections). Each fits a single session (with
the longer two --- 5 and 13 --- nearer the upper bound). Total
Philippic phase is likely 10 sessions left (Phil 3 and Phil 4
now completed, Phil 5--14 to go).
...
3--4 parallel batches of worker sub-agents, each translating
3--5 short letters end-to-end}) remains stale and continues to
auto-pivot. Sessions 32--39 all dropped to single-substantial-
speech mode per this section's instructions; the same
auto-pivot will continue through the Philippic and
philosophical phases (sessions 40--60 or so). Updating the
scheduled-task prompt to reflect Phase 3 of OPERATING\_PLAN.md
(``one substantial-work worker per session, 5-section chunks,
WIP-emit discipline, multi-session splits allowed for very
long works'') is recommended but not blocking --- the PM in
...
## Where to resume now (session 38 legacy snapshot below)

\textbf{Translation state (post-Cowork-session-38):} \textbf{908 /
958 works drafted (\~94.8\%)} --- session 38 completed
\textit{Philippic 3} (20 December 44 BC, 39 sections,
$\sim$5{,}900 English words) end-to-end in one session: Latin
fetched, English translated in single-substantial-speech mode,
headnote written (four paragraphs covering composition history
--- Octavian's break with Antony, the desertion of the Martian
and Fourth Legions, Decimus Brutus's 9 December edict, the
20 December Senate meeting --- and the speech's three-movement
structure: praise-of-Octavian-and-legions / ridicule-of-Antony's
edict-and-flight / hortatory close and motion). Status flipped
\texttt{pending}~$\rightarrow$~\texttt{drafted};
\texttt{backfill\_structural.py}, \texttt{build\_manifest.py}, and
\texttt{validate.py} all run clean. \textbf{Chronological-gap
warning falls from 45 (post-session-37) to 44 (post-session-38)},
the expected $-1$ decrement (Philippic 3 moved out of the gap).
\textbf{Latest drafted by deep chronology is unchanged}:
\textit{Ad Familiares} 10.24 (28 July 43 BC) remains the marker,
since \textit{Philippic 3} is dated 20 December 44 BC and so
falls inside the chronological-gap zone.

\textbf{Cowork session 38 --- 1 substantial speech completed
(1 worker, 39 sections):}

\begin{itemize}
\item \textbf{Worker A (\textit{Philippic 3}, 20 December 44 BC,
the first Philippic delivered after Octavian's break with Antony
and the speech that turned a private quarrel into a Senate-led
war):} 39 sections, $\sim$5{,}900 English words. The Senate was
called by the tribunes of the plebs ostensibly to provide a guard
for the consuls-elect (Pansa and Hirtius, due to enter office on
the Kalends of January). Cicero, who since the Second Philippic
had withdrawn from public business, came in person and turned
the agenda. \S 1--7 the praise of Octavian (\textit{C. Caesar
adulescens, paene potius puer, incredibili ac divina quadam
mente atque virtute}, \S 3) and of the Martian Legion at Alba
under Lucius Egnatuleius and the Fourth Legion's defection.
\S 8--12 the Decimus Brutus / Tarquin comparison: the elder
Brutus expelled a proud king, the younger Brutus shuts out a
king who is cruel, impious, a seller of immunities and
citizenships and provinces. \S 13--14 the consensus of the towns
of Gaul, with the climactic resolution that Antony be judged
``not by word but in fact, not only no consul but a public
enemy.'' \S 15--18 the savage cross-comparison of Antony's
attack on Octavian's mother ``from Aricia'' against Antony's own
wife Fulvia, whose father Bambalio got his cognomen from his
stammer (\textit{cognomen ex contumelia traxerit}). \S 19--24
the collapse of the Senate meeting of 28 November --- the edict
threatening absentees, the \textit{ipse non adfuit} retort, the
mockery of Antony's broken syntax (\textit{en cur magister eius
ex oratore arator factus sit}, \S 22), the news of the Fourth
Legion's defection and the war-cloak flight (\textit{quam misera
fuga, quam foeda, quam turpis}, \S 24). \S 25--27 the catalogue
of consulars who refused the provinces Antony had allotted.
\S 28--36 the hortatory close --- \textit{hodierno die primum,
patres conscripti, longo intervallo in possessionem libertatis
pedem ponimus} (\S 28) and the choice (\S 29) \textit{aut
libertatem propriam Romani et generis et nominis recuperemus
aut mortem servituti anteponamus}. \S 37--39 the motion proper:
guards for the Kalends of January, formal commendation of
Decimus Brutus, the provinces held under the Julian Law, honours
for Octavian and the legions to be referred by the incoming
consuls. \textbf{Sidecar:} parallel-sidecar generated via
\texttt{backfill\_structural.py} (39 segments,
\texttt{data/parallel/speeches/044bc-philippic-3.json}, 63KB).
No Greek phrases in this speech (consistent with the Philippics
generally, which are senatorial in register and bare of the
Greek-tagged informalities Cicero uses with Atticus). No date
correction needed (\texttt{-0044-12-20} day-precision was
already correct).
\end{itemize}

\textbf{No metadata corrections committed during session 38.}
The meta entry was already correct, day-precision.
...
deferred to a future enrichment pass):
\begin{itemize}
\item \textbf{\S 3 \textit{C. Caesar adulescens, paene potius
puer, incredibili ac divina quadam mente atque virtute}.}
Rendered ``Gaius Caesar, a young man --- almost still a boy ---
with an incredible and almost divine mind and courage.''
Preserved the \textit{paene potius puer} self-correction
(Cicero shifting from \textit{adulescens} to the stronger
\textit{puer}); the \textit{divina quadam} kept as ``almost
divine,'' with \textit{quadam} as ``almost'' rather than the
weaker ``some sort of.''
\item \textbf{\S 3 \textit{non enim effudit: in salute rei
publicae conlocavit}.} The famous self-correction (Octavian did
not ``pour out'' his patrimony --- he ``invested'' it). Rendered
``he did not pour it out: he invested it in the safety of the
commonwealth.'' The \textit{effudit / conlocavit} antithesis
(squandering vs. investment) preserved as ``poured out /
invested.''
\item \textbf{\S 8 \textit{O civem natum rei publicae, memorem
sui nominis imitatoremque maiorum!}} Apostrophe to Decimus
Brutus. Rendered ``O citizen born for the commonwealth, mindful
of his name, an imitator of his ancestors!'' The
\textit{memorem / imitatorem} doublet preserved.
\item \textbf{\S 9 \textit{L. Brutus regem superbum non tulit:
D. Brutus sceleratum atque impium regnare patietur Antonium?}}
The two Bruti antithesis. Rendered ``Lucius Brutus would not
endure a proud king: shall Decimus Brutus suffer the criminal
and impious Antony to be king?'' The proper-name parallelism
and the \textit{tulit / patietur} verb-pair preserved.
\item \textbf{\S 10 \textit{huius domi inter quasilla
pendebatur aurum, numerabatur pecunia}.} The wool-basket
image. Rendered ``in this man's house gold was weighed out
among the wool-baskets, money was counted out.'' Kept the
literal \textit{quasilla} (wool-baskets), which is the load-
bearing detail --- Antony's house is run like a women's
work-room, with gold weighed beside the spinning baskets ---
not generalised to ``in his household.''
\item \textbf{\S 14 \textit{non verbo, sed re, non modo non
consul sed etiam hostis Antonius}.} The climactic resolution.
Rendered ``not by word, but in fact, not only no consul but
also a public enemy.'' The double \textit{non $\ldots$ sed}
construction preserved in both members, and \textit{hostis}
rendered as the technical ``public enemy'' rather than the bare
``enemy'' (since the speech's whole point is to push the Senate
toward, but stop just short of, the formal \textit{hostis}
declaration).
\item \textbf{\S 16 \textit{Bambalio quidam pater, homo nullo
numero $\ldots$ cognomen ex contumelia traxerit}.} The savage
attack on Fulvia's father. Rendered ``the father was a certain
Bambalio, a man of no account whatever $\ldots$ got his
cognomen out of insult, on account of the hesitancy of his
tongue and the stupor of his heart.'' \textit{Bambalio} is
itself an onomatopoeic stammer-word (cf.\ Greek
\textit{bambalein}, to chatter the teeth); the
\textit{haesitantia linguae / stupor cordis} doublet preserved.
\item \textbf{\S 16 \textit{vellem hanc contemptionem
pecuniae suis reliquisset!}} The dry aside about Tuditanus.
Rendered ``I could wish he had left his descendants this
contempt for money!'' The understatement (Antony and his
relatives have plainly not inherited the trait) kept implicit,
not glossed.
\item \textbf{\S 21 \textit{quem in edictis Spartacum
appellat, hunc in senatu ne improbum quidem dicere audet}.}
Rendered ``Whom in his edicts he calls a Spartacus, in the
Senate he does not dare even call dishonest.'' The
\textit{Spartacum / improbum} antithesis (extreme insult in
print, no insult at all in person) preserved.
\item \textbf{\S 22 \textit{en cur magister eius ex oratore
arator factus sit}.} The pun on Antony's teacher
(Sextus Clodius?) being given two thousand acres of public
land in the Leontine plain. \textit{Orator / arator} (one
letter's difference, but the whole shift from rhetoric to
ploughing). Rendered ``see why his teacher has been turned from
an orator into a ploughman.'' Considered ``orator into a
tiller,'' but \textit{arator} is the technical agricultural
title and ``ploughman'' lands closer to the colloquial mockery
Cicero is reaching for. The pun cannot survive in English.
\item \textbf{\S 24 \textit{quae vero profectio postea, quod
iter paludati, quae vitatio oculorum, lucis, urbis, fori, quam
misera fuga, quam foeda, quam turpis!}} The tricolon of
\textit{misera / foeda / turpis} closing a longer
\textit{quae vero $\ldots$ quod $\ldots$ quae $\ldots$}
catalogue of Antony's flight. Rendered ``what a setting out
afterwards, what a journey in the war-cloak, what avoiding of
men's eyes, of the light, of the City, of the Forum, what a
wretched flight, how foul, how disgraceful!'' The four
\textit{quae/quod} questions kept as four English ``what''s;
the closing tricolon preserved as ``how foul, how disgraceful''
sealing the longer build.
\item \textbf{\S 27 \textit{O praeclarum custodem ovium, ut
aiunt, lupum!}} The wolf-as-guardian-of-the-sheep aphorism.
Rendered ``O illustrious guardian of the sheep, as men say, a
wolf!'' The \textit{ut aiunt} (``as they say'') marks this as
a current proverb; preserved as ``as men say.'' The Plautine
flavour kept (cf.\ \textit{Pseudolus} 140 \textit{ovem
lupo committere}); worth an allusions entry once enrichment
runs.
\item \textbf{\S 28 \textit{hodierno die primum, patres
conscripti, longo intervallo in possessionem libertatis pedem
ponimus}.} The cadence opening the hortatory peroration.
Rendered ``On this day, senators, for the first time after a
long interval, we plant our foot upon the possession of
liberty.'' The legal-language \textit{in possessionem $\ldots$
pedem ponimus} (a phrase from possessory interdicts, ``we plant
our foot upon possession'') kept literal, since the legal
flavour is the load-bearing point: liberty has been won back as
property is won back, by entry-into-possession.
\item \textbf{\S 28 \textit{hanc vero taeterrimam beluam quis
ferre potest aut quo modo?}} Rendered ``But who can endure this
most foul beast --- and how?'' The \textit{taeterrima belua}
preserved as ``most foul beast'' (the metaphor is Cicero's
recurring one for Antony; ``creature'' or ``monster'' would
lose the load-bearing bestiality).
\item \textbf{\S 29 \textit{aut libertatem propriam Romani et
generis et nominis recuperemus aut mortem servituti
anteponamus}.} The choice. Rendered ``either let us recover
the liberty proper to the Roman race and name, or set death
before slavery.'' The \textit{aut $\ldots$ aut} construction
preserved as ``either $\ldots$ or''; \textit{anteponamus}
rendered ``set before'' rather than ``prefer'' to keep the
literal spatial image (death stationed in front of slavery as
a barrier).
\item \textbf{\S 31 \textit{ille autem ex myrmillone dux, ex
gladiatore imperator}.} Lucius Antonius the brother. Rendered
``that man, from murmillo a leader, from gladiator a
commander.'' Both \textit{myrmillo} (a specific gladiator class
with fish-crest helmet, sword, and shield) and \textit{gladiator}
kept in their Roman cultural specificity --- ``murmillo'' as a
loanword rather than naturalised to ``swordsman.'' The four
substantive nouns in the \textit{ex \ldots dux / ex \ldots
imperator} chiasmus preserved.
\item \textbf{\S 36 \textit{ad decus et ad libertatem nati
sumus: aut haec teneamus aut cum dignitate moriamur}.}
Rendered ``For glory and for liberty we were born: let us
either keep these, or die with dignity.'' The two
\textit{ad \ldots} prepositional phrases preserved as ``For
\ldots and for \ldots''; the closing \textit{aut \ldots aut}
echoes the \S 29 cadence and is preserved.
\item \textbf{\S 36 \textit{iucundiorem autem faciet
libertatem servitutis recordatio}.} The closing aphorism
before the formal motion. Rendered ``The recollection of
slavery will make liberty sweeter.'' \textit{Iucundiorem}
preserved as ``sweeter'' (rather than ``more pleasant''),
which lands the comparative and the affective weight
together.
\end{itemize}

\textbf{Suggested next translation batch} (when session 38 is
landed via \texttt{bash scripts/cowork\_handoff.sh "session 38:
\ldots"} and session 39 opens):

\textbf{Session 39 opens \textit{Philippic 4} (20 December 44
BC, $\sim$16 sections).} The popular counterpart delivered on
the same day in the Forum, of which it shares the arguments at
a higher pitch of indignation. Short enough to fit in a single
session with comfortable margin; the single-substantial-speech
pattern continues (fetch via \texttt{scripts/fetch\_latin.py
philippic-4}, translate in $\sim$5-section chunks, headnote at
close, status-flip and consolidation scripts).

\textbf{After \textit{Philippic 4}} (sessions 40--49 or so):
...
sections), \textit{12} (Mar 43, $\sim$30 sections),
\textit{13} (20 Mar 43, $\sim$50 sections), \textit{14} (21
Apr 43, $\sim$38 sections). Each fits a single session; total
Philippic phase is likely 11 sessions left (Phil 3 just
completed, Phil 4--14 to go).
...
3--4 parallel batches of worker sub-agents, each translating
3--5 short letters end-to-end}) remains stale and continues to
auto-pivot. Sessions 32--38 all dropped to single-substantial-
speech mode per this section's instructions; the same
auto-pivot will continue through the Philippic and
philosophical phases (sessions 39--60 or so). Updating the
scheduled-task prompt to reflect Phase 3 of OPERATING\_PLAN.md
(``one substantial-work worker per session, 5-section chunks,
WIP-emit discipline, multi-session splits allowed for very
long works'') is recommended but not blocking --- the PM in
